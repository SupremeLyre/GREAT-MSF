...
t_gio::t_gio() : _fd(-1), _size(BUF_SIZE), _path(""), _giof(), _count(0), _verb(0), _stop(0), _opened(0), _running(0)
{
    _coder = 0;
}

t_gio::t_gio(t_spdlog spdlog)
    : _fd(-1), _size(BUF_SIZE), _path(""), _giof(), _count(0), _verb(0), _stop(0), _opened(0), _running(0)
{
    if (nullptr == spdlog)
    {
