...
t_gpoly::t_gpoly() : _valid(false), _ncoeff(0), _rms(0.0), _xref(0.0), _span(0.0)
{
    _coef.resize(0);
}
...
int t_gpoly::polynomials(const vector<double> &X, const vector<double> &Y)
{
    ...
    {
        for (int j = 0; j < N; j++)
            cerr << _span << fixed << " N=" << N << " R=" << _xref << " X=" << setprecision(3) << setw(12)
                 << X[j] - _xref << " Y=" << setprecision(6) << setw(16) << Y[j] << " C=" << setprecision(6) << setw(16)
                 << _coef[j] << endl;
    }
    return 0;
}
