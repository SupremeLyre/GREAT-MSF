...
t_gparhead::t_gparhead(par_type type, string site, string sat) : type(type), site(site), sat(sat)
{
}

t_gparhead::t_gparhead(const t_gparhead &Other) : type(Other.type), sat(Other.sat), site(Other.site)
{
}
...
t_gtimearc::t_gtimearc(const t_gtime &beg, const t_gtime &end) : begin(beg), end(end)
{
}
...
double t_gpar::partial(t_gsatdata &satData, t_gtime &epoch, t_gtriple ground, t_gobs &gobs)
{
    ...
    case par_type::AMB_L1:
        if (satData.site() == this->site && gobs.is_phase() &&
            t_gsys::band2freq(satData.gsys(), gobs.band()) == FREQ_1 && prn == satData.sat())
            return 1.0;
        else
            return 0.0;
    case par_type::AMB_L2:
        if (satData.site() == this->site && gobs.is_phase() &&
            t_gsys::band2freq(satData.gsys(), gobs.band()) == FREQ_2 && prn == satData.sat())
            return 1.0;
        else
            return 0.0;
    case par_type::AMB_L3:
        if (satData.site() == this->site && gobs.is_phase() &&
            t_gsys::band2freq(satData.gsys(), gobs.band()) == FREQ_3 && prn == satData.sat())
            return 1.0;
        else
            return 0.0;
    case par_type::AMB_L4:
        if (satData.site() == this->site && gobs.is_phase() &&
            t_gsys::band2freq(satData.gsys(), gobs.band()) == FREQ_4 && prn == satData.sat())
            return 1.0;
        else
            return 0.0;
    case par_type::AMB_L5:
        if (satData.site() == this->site && gobs.is_phase() &&
            t_gsys::band2freq(satData.gsys(), gobs.band()) == FREQ_5 && prn == satData.sat())
            return 1.0;
        else
            return 0.0;
...
void t_gpar::_getmf(t_gsatdata &satData, const t_gtriple &crd, const t_gtime &epoch, double &mfw, double &mfh,
                    double &dmfw, double &dmfh)
{
