...
namespace gnut
{
t_ginterp::t_ginterp() : _interp_1d(LINEAR), _interp_2d(BILINEAR), _interp_3d(VER2HOR), _interp_ht(INTERPOLATE)
{
}

t_ginterp::t_ginterp(t_spdlog spdlog)
    : t_gdata(spdlog), _interp_1d(LINEAR), _interp_2d(BILINEAR), _interp_3d(VER2HOR), _interp_ht(INTERPOLATE)
{
}
...
int t_ginterp::spline(map<double, double> &data, double val, double &fval)
{
    if (data.size() == 1)
    {
        cerr << "Warning: For using of Cubic Spline Interpolation Method, the dimension of input data vector equal to "
                "(at least): dimm = 3, is requested "
             << endl;
        return -1;
    }
    else if (data.size() == 2)
    ...
    else if (data.size() > 2)
    {
        ...
        for (unsigned int i = 0; i < X.size() - 1; i++) // ok
        {
            ...
            FI.push_back(MM[i] * (pow((X[i + 1] - e), 3) / (6 * h[i])) +
                         MM[i + 1] * ((pow((e - X[i]), 3)) / (6 * h[i])) + AA[i] * (e - X[i]) + BB[i]);
            if (e == X[i])
            {
                fval = Y[i];
            }
...
int t_ginterp::linear(const map<t_gtime, double> &data, const t_gtime &epo, double &fval)
{
    ...
    {
        cerr << "# warning: a problem in time linear interpolation! [#node:" << data.size() << "] " << epo.str_ymdhms()
             << endl;
        return -1;
    }
...
int t_ginterp::bilinear(const map<t_gpair, double> &data, const t_gpair &req_pos, double &fval)
{
...
#ifdef DEBUG
        cout << "# interp:" << fixed << setprecision(0) << " " << itDAT->first[0] << ":" << itDAT->first[1] << " "
             << setprecision(3) << " " << data.at(v_pt.at(v_pt.size() - 1)) << endl;
#endif
    }
    ...
    else if (v_pt.size() == 4)
    {
        ...
        cout << "# Linear interpolation (4): " << fixed << setprecision(0) << "  " << v_pt[0][0] << ":" << v_pt[0][1]
             << "  " << v_pt[1][0] << ":" << v_pt[1][1] << "  " << v_pt[2][0] << ":" << v_pt[2][1] << "  " << v_pt[3][0]
             << ":" << v_pt[3][1] << " ." << setprecision(3) << "  " << req_pos[0] << "-" << req_pos[1] << "  "
             << data.at(v_pt[0]) << "  " << data.at(v_pt[1]) << "  " << data.at(v_pt[2]) << "  " << data.at(v_pt[3])
             << endl;
#endif
