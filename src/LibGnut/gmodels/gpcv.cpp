...
{
t_gpcv::t_gpcv()
    : t_gdata(), _trans(true), // transmitter (default yes)
      _anten(""),              // antenna type
...
t_gpcv::t_gpcv(t_spdlog spdlog)
    : t_gdata(spdlog), _trans(true), // transmitter (default yes)
      _anten(""),                    // antenna type
      _ident(""),                    // antenna identification
      _svcod(""),                    // SVN code
      _method(""),                   // calibartion method
...
int t_gpcv::pcoS_cmb(t_gsatdata &satdata, t_gtriple &pco, GOBSBAND &b1, GOBSBAND &b2)
{
    ...
    else
    {
        ...
        {
            if (_mappco.find(f1) != _mappco.end())
                _gnote->mesg(GWARNING, "gpcv",
                             "no SAT PCO [" + _anten + "/" + t_gfreq::gfreq2str(f1) + "], using hardwired values");
            if (_mappco.find(f2) != _mappco.end())
                _gnote->mesg(GWARNING, "gpcv",
                             "no SAT PCO [" + _anten + "/" + t_gfreq::gfreq2str(f2) + "], using hardwired values");
        }
...
int t_gpcv::pcoR_cmb(t_gsatdata &satdata, t_gtriple &pco, GOBSBAND &b1, GOBSBAND &b2)
{
    ...
    if (_mappco.find(f1) == _mappco.end())
    {
        if (_gnote)
            _gnote->mesg(GWARNING, "gpcv",
                         "no REC PCO [" + _anten + "/" + t_gfreq::gfreq2str(f1) + "], substituted with GPS L1");
        f1 = t_gsys::freq_priority(gsys, (t_gsys::gfrq2freq(gsys, f1) <= 2) ? t_gsys::gfrq2freq(gsys, f1) : FREQ_2);
    }
    if (_mappco.find(f2) == _mappco.end())
    {
        if (_gnote)
            _gnote->mesg(GWARNING, "gpcv",
                         "no REC PCO [" + _anten + "/" + t_gfreq::gfreq2str(f2) + "], substituted with GPS L2");
        f2 = t_gsys::freq_priority(gsys, (t_gsys::gfrq2freq(gsys, f2) <= 2) ? t_gsys::gfrq2freq(gsys, f2) : FREQ_2);
    }
...
int t_gpcv::pcvS_cmb(double &corr, t_gsatdata &satdata, GOBSBAND &b1, GOBSBAND &b2, t_gtriple &site)
{
    ...
    if (_mapzen.find(f1) == _mapzen.end())
    {
        if (_gnote)
            _gnote->mesg(GWARNING, "gpcv",
                         "no REC PCV [" + _anten + "/" + t_gfreq::gfreq2str(f1) + "], substituted with GSYS L1");
        f1 = t_gsys::freq_priority(gsys, (t_gsys::gfrq2freq(gsys, f1) <= 2) ? t_gsys::gfrq2freq(gsys, f1) : FREQ_2);
    }
    ...
    if (_mapzen.find(f2) == _mapzen.end())
    {
        if (_gnote)
            _gnote->mesg(GWARNING, "gpcv",
                         "no REC PCV [" + _anten + "/" + t_gfreq::gfreq2str(f2) + "], substituted with GSYS L1");
...
int t_gpcv::pcvR_cmb(double &corr, t_gsatdata &satdata, GOBSBAND &b1, GOBSBAND &b2)
{
    ...
    { // AZI-dependant calibration NOT available (only NOAZI)
        ...
        {
            _gnote->mesg(GWARNING, "gpcv",
                         "no REC AZI PCV [" + _anten + "/freq:" + t_gfreq::gfreq2str(f1) + "], just used NOAZI");
        }
...
int t_gpcv::pcoR_raw(t_gsatdata &satdata, t_gtriple &pco, GOBSBAND &b1)
{
    ...
    if (_mappco.find(f1) == _mappco.end())
    {
        if (_gnote)
            _gnote->mesg(GWARNING, "gpcv",
                         "no REC PCO [" + _anten + "/" + t_gfreq::gfreq2str(f1) + "], substituted with GPS L2");
        f1 = t_gsys::freq_priority(gsys, (t_gsys::gfrq2freq(gsys, f1) <= 2) ? t_gsys::gfrq2freq(gsys, f1) : FREQ_2);
    }
    ...
    else
    {
        if (_gnote)
        {
            ...
            if (_mappco.find(f3) != _mappco.end())
                _gnote->mesg(GWARNING, "gpcv",
                             "no SAT PCO [" + _anten + "/" + t_gfreq::gfreq2str(f3) + "], using hardwired values");
            if (_mappco.find(f4) != _mappco.end())
                _gnote->mesg(GWARNING, "gpcv",
                             "no SAT PCO [" + _anten + "/" + t_gfreq::gfreq2str(f4) + "], using hardwired values");
            if (_mappco.find(f5) != _mappco.end())
                _gnote->mesg(GWARNING, "gpcv",
                             "no SAT PCO [" + _anten + "/" + t_gfreq::gfreq2str(f5) + "], using hardwired values");
        }
...
int t_gpcv::pcvS(double &corrLC, t_gsatdata &satdata, t_gtriple &site, GOBS_LC lc, GOBSBAND k1, GOBSBAND k2)
{
    ...
    if (lc == GOBS_LC::LC_L1 || lc == GOBS_LC::LC_L2 || lc == GOBS_LC::LC_L3 || lc == GOBS_LC::LC_L4 ||
        lc == GOBS_LC::LC_L5)
    {
        switch (lc)
        {
        ...
        case GOBS_LC::LC_L2:
            if (_mapzen.find(f2) == _mapzen.end())
            {
                if (_gnote)
                    _gnote->mesg(GWARNING, "gpcv",
                                 "no REC PCV [" + _anten + "/" + t_gfreq::gfreq2str(f2) + "], substituted with GPS L2");
                f2 = t_gsys::freq_priority(GPS,
                                           (t_gsys::gfrq2freq(gsys, f2) <= 2) ? t_gsys::gfrq2freq(gsys, f2) : FREQ_2);
            }
            if (_mapzen.find(f2) == _mapzen.end())
            {
                _gmutex.unlock();
