...
class LibGnut_LIBRARY_EXPORT t_gsppmodel : public t_gmodel
{
  ...
  protected:
    ...
    double _maxres(bool phase, vector<t_gsatdata> &data, vector<t_gsatdata>::iterator &itDATA, RESIDTYPE res_type,
                   GSYS gs = GNS);

    /** @brief check maximal residual */
    bool _check_outl(bool phase, double &maxresNORM, vector<t_gsatdata>::iterator &itDataNORM, double &maxresORIG,
                     vector<t_gsatdata>::iterator &itDataORIG, vector<t_gsatdata>::iterator &itDataErase,
