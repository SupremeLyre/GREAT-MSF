...
t_gsppmodel::t_gsppmodel(string site, t_gsetbase *settings) : _observ(gnut::OBSCOMBIN::IONO_FREE)
{
...
t_gsppmodel::t_gsppmodel(t_spdlog spdlog, string site, t_gsetbase *settings) : _observ(gnut::OBSCOMBIN::IONO_FREE)
{
    if (nullptr == spdlog)
    {
...
double t_gsppmodel::_maxres(bool phase, vector<t_gsatdata> &data, vector<t_gsatdata>::iterator &itDATA,
                            RESIDTYPE res_type, GSYS gs)
{
                              ...
                              vector<t_gsatdata>::iterator &itDataErase, vector<t_gsatdata> &data)
{
    ...
    if (_resid_type == gnut::RESIDTYPE::RES_ORIG)
    {
        if (maxresORIG > map_res[gs])
        {
            ...
            if (phase)
                _logOutl(true, itDataORIG->sat(), data.size(), maxresORIG, itDataORIG->ele_deg(), itDataORIG->epoch(),
                         _resid_type);
            else
                _logOutl(false, itDataORIG->sat(), data.size(), maxresORIG, itDataORIG->ele_deg(), itDataORIG->epoch(),
                         _resid_type);
            return true;
        }
    }
    else if (_resid_type == gnut::RESIDTYPE::RES_NORM)
    {
        if (maxresNORM > _maxres_norm)
        {
            ...
            if (phase)
                _logOutl(true, itDataNORM->sat(), data.size(), maxresNORM, itDataNORM->ele_deg(), itDataNORM->epoch(),
                         _resid_type);
            else
                _logOutl(false, itDataNORM->sat(), data.size(), maxresNORM, itDataNORM->ele_deg(), itDataNORM->epoch(),
                         _resid_type);
            return true;
        }
    }
    else if (_resid_type == gnut::RESIDTYPE::RES_ALL)
    {
        if (maxresORIG > map_res[gs] || maxresNORM > _maxres_norm)
        {
            if (itDataORIG == itDataNORM)
            {
                itDataErase = itDataORIG;
                if (phase)
                    _logOutl(true, itDataORIG->sat(), data.size(), maxresORIG, itDataORIG->ele_deg(),
                             itDataORIG->epoch(), gnut::RESIDTYPE::RES_ORIG);
                else
                    _logOutl(false, itDataORIG->sat(), data.size(), maxresORIG, itDataORIG->ele_deg(),
                             itDataORIG->epoch(), gnut::RESIDTYPE::RES_ORIG);
                return true;
            }
            else
            {
                ...
                if (ratioNORM >= ratioORIG)
                {
                    itDataErase = itDataNORM;
                    if (phase)
                        _logOutl(true, itDataNORM->sat(), data.size(), maxresNORM, itDataNORM->ele_deg(),
                                 itDataNORM->epoch(), gnut::RESIDTYPE::RES_NORM);
                    else
                        _logOutl(false, itDataNORM->sat(), data.size(), maxresNORM, itDataNORM->ele_deg(),
                                 itDataNORM->epoch(), gnut::RESIDTYPE::RES_NORM);
                    return true;
                }
                else
...
void t_gsppmodel::_logOutl(bool phase, string prn, int data_size, double maxres, double ele, t_gtime epo,
                           RESIDTYPE resid_type)
{
    ...
    ostringstream os;
    os << _site << " outlier (" << resType << ": " << obsType << ") " << prn << " size:" << fixed << setw(2)
       << data_size << " v: " << fixed << setw(16) << right << setprecision(3) << maxres << " ele: " << fixed << setw(6)
       << setprecision(2) << ele << " " << epo.str_hms();
    if (_spdlog)
