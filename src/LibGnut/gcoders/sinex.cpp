...
t_sinex::t_sinex(t_gsetbase *s, string version, int sz, string id)
    : t_gcoder(s, version, sz, id), _snx_type(SINEX_GNS), _technique('P'),
      _parindex(1), // should start from 1 (SINEX format)
      _tmpsize(0), _consume(0), _complete(true), _estimation(false), _code_label("CODE"), // LABEL FOR SITE
      _list_gnss(""), _line(""), _site(""), _block(""), _pco_mod("igsXX_XXXX"), _ac(""), _pt_prod(0),
      _file_beg(t_gtime::GPS), _file_end(t_gtime::GPS), _file_run(t_gtime::GPS), _allobj(0), _end_epoch(false),
      _end_estim(false)
{
}
...
int t_sinex::decode_head(char *buff, int sz, vector<string> &errmsg)
{
    ...
    while ((_tmpsize = t_gcoder::_getline(_line)) >= 0)
    {
        ...
        // 1st line
        // -------- "FILE HEADER" --------
        if (_line.find("%=TRO") != string::npos || _line.find("%=BIA") != string::npos ||
            _line.find("%=SNX") != string::npos)
        {
