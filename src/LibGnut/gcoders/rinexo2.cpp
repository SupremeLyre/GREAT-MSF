...
t_rinexo2::t_rinexo2(t_gsetbase *s, string version, int sz)
    : t_gcoder(s, version, sz), _site(""), _epo_beg(LAST_TIME), _epo_end(FIRST_TIME), _xbeg(0), _xend(0), _xsmp(0),
      _xsys(0)
{
    this->clear();
}
t_rinexo2::t_rinexo2(t_gtime beg, t_gtime end, t_gsetbase *s, string version, int sz)
    : t_gcoder(beg, end, s, version, sz), _site(""), _epo_beg(LAST_TIME), _epo_end(FIRST_TIME), _xbeg(0), _xend(0),
      _xsmp(0), _xsys(0)
{
    this->clear();
}
...
t_rinexo2::~t_rinexo2()
{
    ...
    while (itDAT != _data.end())
    {
        ...
        if (_spdlog)
            SPDLOG_LOGGER_DEBUG(_spdlog, _fname + " [filtered data] xBEG:" + int2str(_xbeg) + " xEND:" +
                                             int2str(_xend) + " xSMP:" + int2str(_xsmp) + " xSYS:" + int2str(_xsys));
...
int t_rinexo2::_decode_head()
{
    ...
    else if (_line.substr(60, 11).find("MARKER NAME") != string::npos)
    {
        ...
        if (!_fname.empty())
        {
            ...
            if (guess != marker)
            {
                ...
                {
                    mesg(GWARNING, "Warning: 4-CHAR: MARKER_NAME [" + marker + "] not equal to site from filename [" +
                                       guess + "]");
                }
            }
        }
        ...
        if (guess.length() == 4)
        {
            for (auto itREC = _rec.begin(); itREC != _rec.end(); ++itREC)
            {
                if (itREC->length() > 4 && itREC->compare(0, 4, guess, 0, 4) == 0)
                {
                    ...
                    if (_spdlog)
                        SPDLOG_LOGGER_DEBUG(_spdlog, "4-CH [" + guess + "] found in RINEX, but user requests 9-CH [" +
                                                         (*itREC) + "]");
                    guess = *itREC;
                }
            }
        ...
        else if (guess.length() == 9)
        {
            for (auto itREC = _rec.begin(); itREC != _rec.end(); ++itREC)
            {
                if (itREC->length() == 4 && itREC->compare(0, 4, guess, 0, 4) == 0)
                {
                    ...
                    if (_spdlog)
                        SPDLOG_LOGGER_DEBUG(_spdlog, "9-CH [" + guess + "] found in RINEX, but user requests 4-CH [" +
                                                         (*itREC) + "]");
                    guess = *itREC;
                }
            }
        ...
        _site = guess;
        _rnxhdr.markname(
            trim(marker)); // capitals, cutted according to the station name guess
    ...
    else if (_line.substr(60, 19).find("REC # / TYPE / VERS") != string::npos)
    {
        ...
        if (_spdlog)
            SPDLOG_LOGGER_DEBUG(_spdlog, "REC # / TYPE / VERS: " + _rnxhdr.recnumb() + " " + _rnxhdr.rectype() + " " +
                                             _rnxhdr.recvers());
    ...
    else if (_line.substr(60, 19).find("APPROX POSITION XYZ") != string::npos)
    {
        ...
        if (_spdlog)
            SPDLOG_LOGGER_DEBUG(_spdlog, "APPROX POSITION XYZ: " + dbl2str(apr[0]) + " " + dbl2str(apr[1]) + " " +
                                             dbl2str(apr[2]));
        _rnxhdr.aprxyz(apr);
    ...
    else if (_line.substr(60, 20).find("ANTENNA: DELTA X/Y/Z") != string::npos)
    {
        ...
        if (_spdlog)
            SPDLOG_LOGGER_DEBUG(_spdlog, "ANTENNA: DELTA X/Y/Z: " + dbl2str(ecc[0]) + " " + dbl2str(ecc[1]) + " " +
                                             dbl2str(ecc[2]));
        _rnxhdr.antxyz(ecc);
    ...
    else if (_line.substr(60, 20).find("ANTENNA: DELTA H/E/N") != string::npos)
    {
        ...
        if (_spdlog)
            SPDLOG_LOGGER_DEBUG(_spdlog, "ANTENNA: DELTA H/E/N: " + dbl2str(ecc[0]) + " " + dbl2str(ecc[1]) + " " +
                                             dbl2str(ecc[2]));
        _rnxhdr.antneu(ecc);
            ...
            if (old_obj == 0)
            {
                ...
                if (_spdlog)
                    SPDLOG_LOGGER_DEBUG(_spdlog,
                                        "Object created, using RINEX header: " + new_obj->id() + epo.str_ymdhms(" "));
            }
            else
            {
...
int t_rinexo2::_fill_data()
{
    ...
    while (itDAT != _data.end())
    {
        ...
        while (itOBS != _vobs.end())
        {
            ...
            if (_spdlog)
                SPDLOG_LOGGER_DEBUG(_spdlog, (*itOBS)->sat() + " obs filled " + (*itOBS)->epoch().str_ymdhms() + " " +
                                                 (*itOBS)->site());
