...
t_grec::t_grec() : t_gobj()
{
    id_type(REC);
    _overwrite = true;
}

t_grec::t_grec(t_spdlog spdlog) : t_gobj(spdlog)
{
    id_type(REC);
    _overwrite = true;
}
    ...
    { // process end according to next value
        ...
        { // only if end is smaller then existing
            ...
            { // too close to next record
                if (_spdlog)
                    SPDLOG_LOGGER_DEBUG(_spdlog, "Warning: " + _id + " 'rec' end tied to the existing value " +
                                                     end.str("%Y-%m-%d %H:%M:%S -> ") +
        ...
        else if (end != it->first)
        {
            ...
            if (_spdlog)
                SPDLOG_LOGGER_DEBUG(_spdlog, "Warning: object " + _id + " 'rec' " + rec +
                                                 " end cut and tied to the existing value " +
