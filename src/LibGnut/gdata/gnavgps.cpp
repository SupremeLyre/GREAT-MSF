...
t_gnavgps::t_gnavgps()
    : t_gnav(), _iode(0), _iodc(0), _health(0), _toe(t_gtime::GPS), _toc(t_gtime::GPS), _tot(t_gtime::GPS), _a(0.0),
      _e(0.0), _m(0.0), _i(0.0), _idot(0.0), _omega(0.0), _OMG(0.0), _OMGDOT(0.0), _dn(0.0), _crc(0.0), _cic(0.0),
      _cuc(0.0), _crs(0.0), _cis(0.0), _cus(0.0), _f0(0.0), _f1(0.0), _f2(0.0), _acc(0), _fit(0.0), _code(0.0),
      _flag(0.0)
{
...
t_gnavgps::t_gnavgps(t_spdlog spdlog)
    : t_gnav(spdlog), _iode(0), _iodc(0), _health(0), _toe(t_gtime::GPS), _toc(t_gtime::GPS), _tot(t_gtime::GPS),
      _a(0.0), _e(0.0), _m(0.0), _i(0.0), _idot(0.0), _omega(0.0), _OMG(0.0), _OMGDOT(0.0), _dn(0.0), _crc(0.0),
      _cic(0.0), _cuc(0.0), _crs(0.0), _cis(0.0), _cus(0.0), _f0(0.0), _f1(0.0), _f2(0.0), _acc(0), _fit(0.0),
      _code(0.0), _flag(0.0)
{
...
string t_gnavgps::line() const
{
    ...
    tmp << " " << setw(3) << sat() << " " << _toc.str("%Y-%m-%d %H:%M:%S") << scientific << setprecision(12) << setw(w)
        << _f0 << setw(w) << _f1 << setw(w) << _f2 << setw(w) << _iode / 1.0 << setw(w) << _iodc / 1.0 << setw(w)
        << _acc << setw(w) << _health / 1.0 << setw(w) << _code << setw(w) << _flag << setw(w) << SQRT(_a) << setw(w)
        << _e << setw(w) << _i << setw(w) << _m << setw(w) << _idot << setw(w) << _omega << setw(w) << _OMG << setw(w)
        << _OMGDOT << setw(w) << _dn << setw(w) << _crc << setw(w) << _cic << setw(w) << _cuc << setw(w) << _crs
        << setw(w) << _cis << setw(w) << _cus << setw(w) << _fit << setw(w) << _tgd[0] << setw(w) << _tgd[1] << setw(w)
        << _tgd[2] << setw(w) << _tgd[3] << endl;
...
string t_gnavgps::linefmt() const
{
    ...
    tmp << " " << setw(3) << sat() << fixed << " " << _toc.str("%Y-%m-%d %H:%M:%S") << fixed << setprecision(0)
        << setw(8) << _tot - _toc << setw(8) << _tot - _toe << setw(4) << _iode << setw(4) << _health << " |" << setw(9)
...
int t_gnavgps::chk(set<string> &msg)
{
    ...
    {
        msg.insert("Issue: " + _sat + " nav large difference [tot-toc] " + dbl2str(_tot - _toc) + " s " +
                   _tot.str_ymdhms() + _toc.str_ymdhms(" "));
        _validity = false;
    }

    if ((_tot - _toe) > 86400 || (_toe - _tot) > t_gnav::nav_validity(GPS))
    {
        msg.insert("Issue: " + _sat + " nav large difference [tot-toe] " + dbl2str(_tot - _toe) + " s " +
                   _tot.str_ymdhms() + _toe.str_ymdhms(" "));
        _validity = false;
    }
    ...
    if (sod_frac == 0 || sod_frac == 7184 || sod_frac == 7168 || sod_frac == 7152 || sod_frac == 7136 ||
        sod_frac == 7120)
    {
    }
    else
...
int t_gnavgps::pos(const t_gtime &t, double xyz[], double var[], double vel[], bool chk_health)
{
    ...
    if (chk_health && _healthy() == false)
    {
        if (_spdlog)
            SPDLOG_LOGGER_DEBUG(_spdlog,
                                "not healthy sat " + sat() + " excluded from pos calculation " + t.str_ymdhms());
    ...
    if (vel)
    {
        ...
        vel[0] = cosOMG * dotx - cosI * sinOMG * doty - x * sinOMG * dotom - y * cosI * cosOMG * dotom +
                 y * sinI * sinOMG * doti;

        vel[1] = sinOMG * dotx + cosI * cosOMG * doty + x * cosOMG * dotom - y * cosI * sinOMG * dotom -
                 y * sinI * cosOMG * doti;
