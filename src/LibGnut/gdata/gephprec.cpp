...
namespace gnut
{
t_gephprec::t_gephprec() : t_geph(), _degree(0), _poly_x(), _poly_y(), _poly_z(), _poly_c()
{
    id_type(t_gdata::EPHPREC);
    id_group(t_gdata::GRP_EPHEM);
}

t_gephprec::t_gephprec(t_spdlog spdlog) : t_geph(spdlog), _degree(0), _poly_x(), _poly_y(), _poly_z(), _poly_c()
{
    id_type(t_gdata::EPHPREC);
    id_group(t_gdata::GRP_EPHEM);
}
...
int t_gephprec::add(string sat, vector<t_gtime> t, const vector<double> &x, const vector<double> &y,
                    const vector<double> &z, const vector<double> &c)
{
    _gmutex.lock();

    // check dimensions
    if (ndata() != t.size() || ndata() != x.size() || ndata() != y.size() || ndata() != z.size() ||
    ...
    if (!_valid_crd() || sat().empty() || fabs(_epoch - t) > (fabs(_poly_x.xref() - _poly_x.span() / 2) + 0.25))
    {
        if (_spdlog)
            SPDLOG_LOGGER_WARN(_spdlog, "no ephemeris [" + _sat + t.str("] %Y-%m-%d %H:%M:%S") +
                                            " epoch: " + _epoch.str("%Y-%m-%d %H:%M:%S") + " tdiff: " + dbl2str(tdiff) +
                                            " xref: " + dbl2str(_poly_x.xref()) + " span: " + dbl2str(_poly_x.span()));
        _gmutex.unlock();
        return -1;
    }
