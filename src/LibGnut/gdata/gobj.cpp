...
namespace gnut
{
t_gobj::t_gobj() : t_gdata(), _id(""), _name(""), _overwrite(false)
{
...
t_gobj::t_gobj(t_spdlog spdlog) : t_gdata(spdlog), _id(""), _name(""), _overwrite(false)
{
    ...
    { // process end according to next value
        ...
        { // only if end is smaller then existing
            ...
            { // too close to next record
                string lg = "Warning: object " + _id + " 'eccxyz' end tied to the existing value " +
...
void t_gobj::_eccneu(const t_gtriple &ecc, const t_gtime &beg, const t_gtime &end)
{
    ...
    { // process end according to next value
        ...
        { // only if end is smaller then existing
            ...
            { // too close to next record
                string lg = "Warning: object " + _id + " 'eccneu' end tied to the existing value " +
                            end.str("%Y-%m-%d %H:%M:%S -> ") + it->first.str("%Y-%m-%d %H:%M:%S");
...
void t_gobj::_crd(const t_gtriple &crd, const t_gtriple &std, const t_gtime &beg, const t_gtime &end, bool overwrite)
{
    ...
    { // process end according to next value
        ...
        { // only if end is smaller then existing
            ...
            { // too close to next record
                string lg = "Warning: object " + _id + " 'crd' end tied to the existing value " +
                            end.str("%Y-%m-%d %H:%M:%S -> ") + it->first.str("%Y-%m-%d %H:%M:%S");
...
void t_gobj::_pcv(shared_ptr<t_gpcv> pcv, const t_gtime &beg, const t_gtime &end)
{
    ...
    { // process end according to next value
        ...
        { // only if end is smaller then existing
            ...
            {
                // too close to next record
                string lg = "Warning: object " + _id + " 'pcv' end tied to the existing value " +
                            end.str("%Y-%m-%d %H:%M:%S -> ") + it->first.str("%Y-%m-%d %H:%M:%S");
        ...
        {
            string lg = "Warning: object " + _id + " 'pcv' end cut and tied to the existing value " +
                        end.str("%Y-%m-%d %H:%M:%S -> ") + it->first.str("%Y-%m-%d %H:%M:%S");
...
shared_ptr<t_gpcv> t_gobj::_pcv(const t_gtime &t) const
{
    ...
    if (it == _mapant.begin())
    {
        ...
        if (_spdlog)
            SPDLOG_LOGGER_DEBUG(_spdlog,
                                "Warning: unknown PCO (no antenna found in the object " + _id + " ) " + t.str_ymdhms());
        return _pcvnull;
    }
    else
    {
    ...
    if (it2 == _mappcv.begin())
    {
        ...
        if (_spdlog)
            SPDLOG_LOGGER_DEBUG(_spdlog,
                                "Warning: unknown PCO ( antenna " + ant + " not found in ATX ) " + t.str_ymdhms());
        return _pcvnull;
    }
    else
    {
    ...
    if (pcv->pcvkey().compare(ant) != 0)
    {
        ...
        if (trim(pcv->pcvkey().substr(0, 16)) == trim(ant.substr(0, 16)))
        {
            ...
            if (_spdlog)
                SPDLOG_LOGGER_DEBUG(_spdlog,
                                    "Warning: PCO Used without considering randome " + pcv->pcvkey() + " " + ant);
        }
        else
        {

            if (_spdlog)
                SPDLOG_LOGGER_DEBUG(_spdlog, "Warning: unknown PCO ( changed antenna " + ant + " not found in ATX ) " +
                                                 t.str_ymdhms());
            return _pcvnull;
        }
    }
...
void t_gobj::_ant(string ant, const t_gtime &beg, const t_gtime &end)
{
    ...
    { // process end according to next value
        ...
        { // only if end is smaller then existing
            ...
            { // too close to next record
                string lg = "Warning: object " + _id + " 'obj' end tied to the existing value " +
                            end.str("%Y-%m-%d %H:%M:%S -> ") + it->first.str("%Y-%m-%d %H:%M:%S");
...
void t_gobj::sync_pcv(t_gallpcv *pcvs)
{
    ...
    else
    {
        for (vector<t_gtime>::iterator it = vant.begin(); it != vant.end(); it++)
        {
            ...
            {
                if (_spdlog)
                    SPDLOG_LOGGER_DEBUG(_spdlog, "Warning: unknown PCO ( antenna " + ant + " not found in ATX ) " +
                                                     epo.str_ymdhms());
            }
        }
    }
...
void t_gobj::compare(shared_ptr<t_gobj> gobj, const t_gtime &tt, string source)
{
    ...
    if (old != alt && !alt.empty())
    {
        if (old.empty())
        {
            ...
            if (_spdlog)
                SPDLOG_LOGGER_DEBUG(_spdlog, "Warning: object " + _id + " completed by " + source + " (Domes): " + alt +
                                                 " (" + tt.str_ymdhms() + ")");
        }
        else if (_overwrite)
        {
            ...
            if (_spdlog)
                SPDLOG_LOGGER_DEBUG(_spdlog, "Warning: object " + _id + " modified  by " + source + " (Domes): " + old +
                                                 " -> " + alt + " (" + tt.str_ymdhms() + ")");
        }
        else
        {
            ...
            if (_spdlog)
                SPDLOG_LOGGER_DEBUG(_spdlog, "Warning: object " + _id + " does not match " + source +
                                                 " (Domes): " + old + " -> " + alt + " (" + tt.str_ymdhms() + ")");
        }
    }
    ...
    if (old != alt && !alt.empty())
    {
        if (old.empty())
        {
            ...
            if (_spdlog)
                SPDLOG_LOGGER_DEBUG(_spdlog, "Warning: object " + _id + " completed by " + source + " (Antenna): " +
                                                 alt + " (" + beg.str_ymdhms() + "->" + end.str_ymdhms() + ")");
        }
        else if (_overwrite)
        {
            _ant(alt, beg, end);
            ...
            if (_spdlog)
                SPDLOG_LOGGER_DEBUG(_spdlog, "Warning: object " + _id + " modified  by " + source +
                                                 " (Antenna): " + old + " -> " + alt + " (" + beg.str_ymdhms() + "->" +
                                                 end.str_ymdhms() + ")");
        }
        else
        {
            ...
            if (_spdlog)
                SPDLOG_LOGGER_DEBUG(_spdlog, "Warning: object " + _id + " setting does not match " + source +
                                                 " (Antenna): " + old + " -> " + alt + " (" + tt.str_ymdhms() + ")");
        }
    }
    ...
    if ((dist > 10 || std_alt < std_old) && !trip_alt.zero())
    {
        if (trip_old.zero() || std_alt < std_old)
        {
            ...
            if (_spdlog)
                SPDLOG_LOGGER_DEBUG(_spdlog, "Warning: object " + _id + " completed by " + source +
                                                 " (Coordinates): " + dbl2str(trip_alt[0]) + " " +
                                                 dbl2str(trip_alt[1]) + " " + dbl2str(trip_alt[2]) + " (" +
        ...
        else if (_overwrite)
        {
            ...
            if (_spdlog)
                SPDLOG_LOGGER_DEBUG(
                    _spdlog, "Warning: object " + _id + " modified  by " + source +
                                 " (Coordinates): " + dbl2str(trip_old[0]) + " " + dbl2str(trip_old[1]) + " " +
                                 dbl2str(trip_old[2]) + " -> " + dbl2str(trip_alt[0]) + " " + dbl2str(trip_alt[1]) +
        ...
        else
        {
            ...
            if (_spdlog)
                SPDLOG_LOGGER_DEBUG(_spdlog, "Warning: object " + _id + " does not match " + source + " (Coordinates)" +
                                                 " (" + tt.str_ymdhms() + ")");
        }
    }
