...
namespace gnut
{
t_gnavglo::t_gnavglo() : t_gnav(), _iodc(0), _toc(t_gtime::UTC)
{
...
t_gnavglo::t_gnavglo(t_spdlog spdlog) : t_gnav(spdlog), _iodc(0), _toc(t_gtime::UTC)
{
...
int t_gnavglo::chk(set<string> &msg)
{
    ...
    double clkB[1] = {0.0};
    double clkF[1] = {0.0};
    if (_clk(_toc - _maxEphAge, clkB) < 0 || _clk(_toc + _maxEphAge, clkF) < 0 ||
        fabs(*clkF - *clkB) * CLK_GLO_FACTOR > MAX_GLO_CLKDIF)
    {
        msg.insert("Issue: " + _sat + " nav not correct [clk] " + _toc.str_ymdhms() +
                   dbl2str(fabs(*clkF - *clkB) * CLK_GLO_FACTOR));
        _validity = false;
    }
    *clkB *= CLK_GLO_FACTOR;
    *clkF *= CLK_GLO_FACTOR;
...
int t_gnavglo::_pos(const t_gtime &t, double xyz[], double var[], double vel[], bool chk_health)
{
    ...
    {
        if (_spdlog)
            SPDLOG_LOGGER_DEBUG(_spdlog, "CRD " + _sat +
                                             ": The user time and GLONASS ephemerides epoch differ too much. TOC: " +
...
int t_gnavglo::_clk(const t_gtime &t, double *clk, double *var, double *dclk, bool chk_health)
{
    ...
    {
        if (_spdlog)
            SPDLOG_LOGGER_DEBUG(_spdlog, "CLK " + _sat +
                                             ": The user time and GLONASS ephemerides epoch differs too much. TOC: " +
                                             _toc.str_ymdhms() + " T: " + t.str_ymdhms());
        return -1;
    }
...
string t_gnavglo::line() const
{
    ...
    tmp << " " << setw(3) << sat() << " " << _toc.str("%Y-%m-%d %H:%M:%S") << scientific << setprecision(12) << setw(w)
        << _tau << setw(w) << _gamma << setw(w) << _tki << setw(w) << _x << setw(w) << _x_d << setw(w) << _x_dd
        << setw(w) << _health << setw(w) << _y << setw(w) << _y_d << setw(w) << _y_dd << setw(w) << _freq_num << setw(w)
        << _z << setw(w) << _z_d << setw(w) << _z_dd << setw(w) << _E;
...
string t_gnavglo::linefmt() const
{
    ...
    tmp << " " << setw(3) << sat() << fixed << " "
        ...
        << setw(8) << _E << setw(8) << _freq_num << setw(4) << _iodc << setw(4) << _health << " |" << setw(12)
        << setprecision(3) << _tau * 1e9                   //   [sec]
...
ColumnVector t_gnavglo::_deriv(const ColumnVector &xx, const t_gtriple &acc)
{
    ...
    xxdot(2) = vel[1];
    xxdot(3) = vel[2];
    xxdot(4) = k1 * crd[0] + k2 * (1.0 - 5.0 * crd[2] * crd[2] / (r * r)) * crd[0] + OMEGA * OMEGA * crd[0] +
               2 * OMEGA * vel[1] + acc[0];
    xxdot(5) = k1 * crd[1] + k2 * (1.0 - 5.0 * crd[2] * crd[2] / (r * r)) * crd[1] + OMEGA * OMEGA * crd[1] -
               2 * OMEGA * vel[0] + acc[1];
