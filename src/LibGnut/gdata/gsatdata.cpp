...
t_gsatdata::t_gsatdata(t_spdlog spdlog, const string &site, const string &sat, const t_gtime &t)
    : t_gobsgnss(spdlog, site, sat, t), _satcrd(0.0, 0.0, 0.0), _satpco(0.0, 0.0, 0.0), _dloudx(0.0, 0.0, 0.0),
      _drate(0.0), _clk(0.0), _dclk(0.0), _ele(0.0), _azi_rec(0.0), _rho(0.0), _eclipse(false), _mfH(0.0), _mfW(0.0),
      _mfG(0.0), _wind(0.0), _low_prec(false), _slipf(false), _beta_val(999), _orb_angle_val(999), _yaw(999)
...
t_gsatdata::t_gsatdata(const t_gobsgnss &obs)
    : t_gobsgnss(obs), // t_gobsgnss(obs.site() ,obs.sat(), obs.epoch() ),
      _satcrd(0.0, 0.0, 0.0), _satpco(0.0, 0.0, 0.0), _clk(0.0), _ele(0.0), _azi_rec(0.0), _rho(0.0), _eclipse(false),
      _mfH(0.0), _mfW(0.0), _mfG(0.0), _wind(0.0), _low_prec(false), _slipf(false), _beta_val(999), _orb_angle_val(999),
      _drate(0.0), _yaw(999)
{
    _spdlog = (obs.spdlog());
    id_type(t_gdata::SATDATA);
...
int t_gsatdata::_addprd(t_gallnav *gnav, const bool &corrTOT, const bool &msk_health)
{
        ...
        if (bands.size() < 1)
        {
            if (_spdlog)
                SPDLOG_LOGGER_DEBUG(_spdlog,
                                    "At least two bands are necessary for TOT correction in sat pos/clk calculation!");
            return -1;
        }
    ...
    if (gnav == 0)
    {
        if (_spdlog)
            SPDLOG_LOGGER_DEBUG(_spdlog,
                                " satellite " + satname + _epoch.str_ymdhms("  t_gallnav pointer is not available "));
        return -1;
    }
    ...
    if (satname.substr(0, 1) != "G" && satname.substr(0, 1) != "R" && satname.substr(0, 1) != "E" &&
        satname.substr(0, 1) != "J" && satname.substr(0, 1) != "C")
    {
        if (_spdlog)
    ...
    else
    {
        ...
        if (irc < 0)
        {
            if (_spdlog)
                SPDLOG_LOGGER_DEBUG(_spdlog,
                                    " satelite " + satname + _epoch.str_ymdhms(" clocks not calculated for epoch "));
            return -1;
        }
    }
    ...
    if (irc < 0)
    {
        if (_spdlog)
            SPDLOG_LOGGER_DEBUG(_spdlog,
                                " satelite " + satname + _epoch.str_ymdhms(" coordinates not calculated for epoch "));
        return -1;
    }
    ...
    if (gs != GLO || (gs == GLO && gnav->id_type() == t_gdata::ALLPREC))
    {
        ...
        {
            if (_spdlog)
                SPDLOG_LOGGER_ERROR(_spdlog, " satelite " + satname +
