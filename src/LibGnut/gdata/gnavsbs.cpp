...
t_gnavsbs::t_gnavsbs() : t_gnav(), _toc(t_gtime::GPS)
{
    id_type(t_gdata::EPHSBS);
    id_group(t_gdata::GRP_EPHEM);
}

t_gnavsbs::t_gnavsbs(t_spdlog spdlog) : t_gnav(spdlog), _toc(t_gtime::GPS)
{
    ...
    if (fabs(Tk) > MAX_SBS_TIMEDIFF)
    {
        if (_spdlog)
            SPDLOG_LOGGER_DEBUG(_spdlog, "CRD " + _sat +
                                             ": The user time and Glonass ephemerides epoch differ too much. TOE: " +
    ...
    if (fabs(Tk) > MAX_SBS_TIMEDIFF)
    {
        if (_spdlog)
            SPDLOG_LOGGER_DEBUG(_spdlog, "CLK " + _sat +
                                             ": The user time and SBAS ephemerides epoch differs too much. TOE: " +
                                             _toc.str_ymdhms() + " T: " + t.str_ymdhms());
        _gmutex.unlock();
        return -1;
    }
...
string t_gnavsbs::line() const
{
    ...
    tmp << " " << setw(3) << sat() << " " << _toc.str("%Y-%m-%d %H:%M:%S") << scientific << setprecision(12) << setw(w)
        << _f0 << setw(w) << _f1 << setw(w) << _tki << setw(w) << _x << setw(w) << _x_d << setw(w) << _x_dd << setw(w)
        << _health << setw(w) << _y << setw(w) << _y_d << setw(w) << _y_dd << setw(w) << _C_rms << setw(w) << _z
        << setw(w) << _z_d << setw(w) << _z_dd << setw(w) << _iod;
...
string t_gnavsbs::linefmt() const
{
    ...
    tmp << " " << setw(3) << sat() << fixed << " " << _toc.str("%Y-%m-%d %H:%M:%S") << fixed << setprecision(0)
        ...
        << setw(8) << 0 << setw(8) << _C_rms << setw(4) << _iod << setw(4) << _health << " |" << setw(12)
        << setprecision(3) << _f0 * 1e9                    //   [sec]
