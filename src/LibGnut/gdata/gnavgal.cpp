...
t_gnavgal::t_gnavgal()
    : t_gnav(), _iode(0), _health(0), _toe(t_gtime::GAL), _toc(t_gtime::GAL), _tot(t_gtime::GAL), _a(0.0), _e(0.0),
      _m(0.0), _i(0.0), _idot(0.0), _omega(0.0), _OMG(0.0), _OMGDOT(0.0), _dn(0.0), _crc(0.0), _cic(0.0), _cuc(0.0),
      _crs(0.0), _cis(0.0), _cus(0.0), _f0(0.0), _f1(0.0), _f2(0.0), _source(0), _sisa(0.0)
{
    _tgd[0] = _tgd[1] = 0.0;
...
t_gnavgal::t_gnavgal(t_spdlog spdlog)
    : t_gnav(spdlog), _iode(0), _health(0), _toe(t_gtime::GAL), _toc(t_gtime::GAL), _tot(t_gtime::GAL), _a(0.0),
      _e(0.0), _m(0.0), _i(0.0), _idot(0.0), _omega(0.0), _OMG(0.0), _OMGDOT(0.0), _dn(0.0), _crc(0.0), _cic(0.0),
      _cuc(0.0), _crs(0.0), _cis(0.0), _cus(0.0), _f0(0.0), _f1(0.0), _f2(0.0), _source(0), _sisa(0.0)
{
    _tgd[0] = _tgd[1] = 0.0;
...
string t_gnavgal::line() const
{
    ...
    tmp << " " << setw(3) << sat() << " " << _toc.str("%Y-%m-%d %H:%M:%S") << scientific << setprecision(12) << setw(w)
        << _f0 << setw(w) << _f1 << setw(w) << _f2 << setw(w) << _iode / 1.0 << setw(w) << _health / 1.0 << setw(w)
        << SQRT(_a) << setw(w) << _e << setw(w) << _i << setw(w) << _m << setw(w) << _idot << setw(w) << _omega
        << setw(w) << _OMG << setw(w) << _OMGDOT << setw(w) << _dn << setw(w) << _crc << setw(w) << _cic << setw(w)
        << _cuc << setw(w) << _crs << setw(w) << _cis << setw(w) << _cus << setw(w) << _tgd[0] // E5a/E1
...
string t_gnavgal::linefmt() const
{
    ...
    tmp << " " << setw(3) << sat() << fixed << " " << _toc.str("%Y-%m-%d %H:%M:%S") << fixed << setprecision(0)
        << setw(8) << _tot - _toc << setw(8) << _tot - _toe << setw(4) << _iode << setw(4) << _health << setw(4)
        << _source << " |" << setw(9) << setprecision(0) << _a * 1e0 // 1 [m]
        ...
        << " |" << setw(11) << health_bin << setw(11) << source_bin << setw(4) << src(true) << setw(9)
        << gnavtype2str(_gnavtype(true));
...
int t_gnavgal::clk(const t_gtime &t, double *clk, double *var, double *dclk, bool chk_health)
{
    ...
    if (chk_health && _healthy() == false)
    {
        if (_spdlog)
            SPDLOG_LOGGER_DEBUG(_spdlog,
                                "not healthy sat " + sat() + " excluded from clk calculation " + t.str_ymdhms());
        return -1; // HEALTH NOT OK
    }
