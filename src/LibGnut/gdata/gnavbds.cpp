...
t_gnavbds::t_gnavbds()
    : t_gnav(), _iode(0), _iodc(0), _health(0), _toe(t_gtime::BDS), _toc(t_gtime::BDS), _tot(t_gtime::BDS), _a(0.0),
...
t_gnavbds::t_gnavbds(t_spdlog spdlog)
    : t_gnav(spdlog), _iode(0), _iodc(0), _health(0), _toe(t_gtime::BDS), _toc(t_gtime::BDS), _tot(t_gtime::BDS),
...
string t_gnavbds::line() const
{
    ...
    tmp << " " << setw(3) << sat() << " " << _toc.str("%Y-%m-%d %H:%M:%S") << scientific << setprecision(12) << setw(w)
        ...
        << setw(w) << _tgd[0] << setw(w) << _tgd[1] << endl;
