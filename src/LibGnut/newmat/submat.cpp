...
GetSubMatrix BaseMatrix::submatrix(int first_row, int last_row, int first_col, int last_col) const
{
    REPORT
    Tracer tr("submatrix");
    int a = first_row - 1;
...
void GetSubMatrix::operator=(const BaseMatrix &bmx)
{
    ...
    Try
    {
        ...
        if (row_number != gmx->Nrows() || col_number != gmx->Ncols())
            Throw(IncompatibleDimensionsException());
        LoadAndStoreFlag lasf = (row_skip == col_skip && gm->type().is_symmetric() && gmx->type().is_symmetric())
