...
inline void RightCircularUpdateCholesky(UpperTriangularMatrix &chol, int k, int l)
{
    right_circular_update_Cholesky(chol, k, l);
}
...
inline void LeftCircularUpdateCholesky(UpperTriangularMatrix &chol, int k, int l)
{
    left_circular_update_Cholesky(chol, k, l);
}

int LibGnut_LIBRARY_EXPORT SVD(const Matrix &, DiagonalMatrix &, Matrix &, Matrix &, bool = true, bool = true);

void LibGnut_LIBRARY_EXPORT SVD(const Matrix &, DiagonalMatrix &);

inline void SVD(const Matrix &A, DiagonalMatrix &D, Matrix &U, bool withU = true)
{
...
inline void EigenValues(const SymmetricMatrix &A, DiagonalMatrix &D, SymmetricMatrix &S)
{
    eigenvalues(A, D, S);
}
...
inline void SortAscending(GeneralMatrix &gm)
{
    sort_ascending(gm);
}

inline void SortDescending(GeneralMatrix &gm)
{
    sort_descending(gm);
}
