...
void right_circular_update_Cholesky(UpperTriangularMatrix &chol, int k, int l)
{
    ...
    for (i = l; i > k; i--)
    {
        ...
        columnL(i - 1) = pythag(columnL(i - 1), columnL(i), cGivens(givensIndex), sGivens(givensIndex));
        columnL(i) = 0.0;
    }
    ...
    for (j = k + 1; j <= nRC; ++j)
    {
        ...
        for (int gIndex = imin; gIndex <= nGivens; ++gIndex)
        {
            ...
            GivensRotationR(cGivens(gIndex), sGivens(gIndex), columnJ(topRowIndex), columnJ(topRowIndex + 1));
        }
