...
static void GR_1D_FS(int PTS, int N_SYM, int N_UN_SYM, const SimpleIntArray &SYM, int P_SYM,
                     const SimpleIntArray &UN_SYM, Real *X, Real *Y);
...
static void R_5_FTK(int N, int M, Real *X0, Real *Y0, Real *X1, Real *Y1, Real *X2, Real *Y2, Real *X3, Real *Y3,
                    Real *X4, Real *Y4);
static void R_8_FTK(int N, int M, Real *X0, Real *Y0, Real *X1, Real *Y1, Real *X2, Real *Y2, Real *X3, Real *Y3,
                    Real *X4, Real *Y4, Real *X5, Real *Y5, Real *X6, Real *Y6, Real *X7, Real *Y7);
static void R_16_FTK(int N, int M, Real *X0, Real *Y0, Real *X1, Real *Y1, Real *X2, Real *Y2, Real *X3, Real *Y3,
                     Real *X4, Real *Y4, Real *X5, Real *Y5, Real *X6, Real *Y6, Real *X7, Real *Y7, Real *X8, Real *Y8,
                     Real *X9, Real *Y9, Real *X10, Real *Y10, Real *X11, Real *Y11, Real *X12, Real *Y12, Real *X13,
                     Real *Y13, Real *X14, Real *Y14, Real *X15, Real *Y15);
...
static void GR_1D_FT(int N, int N_FACTOR, const SimpleIntArray &FACTOR, Real *X, Real *Y)
{
    ...
    for (int i = 0; i < N_FACTOR; i++)
    {
        ...
        case 8:
            REPORT
            R_8_FTK(N, M, X, Y, X + M, Y + M, X + 2 * M, Y + 2 * M, X + 3 * M, Y + 3 * M, X + 4 * M, Y + 4 * M,
                    X + 5 * M, Y + 5 * M, X + 6 * M, Y + 6 * M, X + 7 * M, Y + 7 * M);
            break;
        case 16:
            REPORT
            R_16_FTK(N, M, X, Y, X + M, Y + M, X + 2 * M, Y + 2 * M, X + 3 * M, Y + 3 * M, X + 4 * M, Y + 4 * M,
                     X + 5 * M, Y + 5 * M, X + 6 * M, Y + 6 * M, X + 7 * M, Y + 7 * M, X + 8 * M, Y + 8 * M, X + 9 * M,
                     Y + 9 * M, X + 10 * M, Y + 10 * M, X + 11 * M, Y + 11 * M, X + 12 * M, Y + 12 * M, X + 13 * M,
                     Y + 13 * M, X + 14 * M, Y + 14 * M, X + 15 * M, Y + 15 * M);
            break;
        default:
...
MultiRadixCounter::MultiRadixCounter(int nx, const SimpleIntArray &rx, SimpleIntArray &vx)
    : Radix(rx), Value(vx), n(nx), reverse(0), product(1), counter(0), finish(false)
{
