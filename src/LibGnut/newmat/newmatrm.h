...
class RectMatrixRowCol
{
  ...
  public:
    ...
    RectMatrixRowCol(Real *st, int nx, int sp, int sh) : store(st), n(nx), spacing(sp), shift(sh)
    {
    }
...
{
  public:
    RectMatrixDiag(const DiagonalMatrix &D) : RectMatrixRowCol(D.Store(), D.Nrows(), 1, 1)
    {
    }
    Real &operator[](int i)
    {
