...
t_gtime::t_gtime(const t_tsys &ts) : _mjd(0), _sod(0), _dsec(0.0), _tsys(UTC)
{
...
t_gtime::t_gtime(const time_t &tt, const double &ds, const t_tsys &ts) : _mjd(0), _sod(0), _dsec(0.0), _tsys(UTC)
{
...
t_gtime::t_gtime(const int &yr, const int &mn, const int &dd, const int &hr, const int &mi, const int &sc,
...
t_gtime::t_gtime(const int &gw, const double &sow, const t_tsys &ts) : _mjd(0), _sod(0), _dsec(0.0), _tsys(ts)
{
...
int t_gtime::from_time(const time_t &tt, const double &ds, const bool &conv)
{
    ...
    from_ymd(tm->tm_year + 1900, tm->tm_mon + 1, tm->tm_mday, tm->tm_hour * 3600 + tm->tm_min * 60 + tm->tm_sec, ds,
             conv);
...
int t_gtime::from_ymdhms(const int &yr, const int &mn, const int &dd, const int &h, const int &m, const double &s,
                         const bool &conv)
{
    int year(yr);
    _norm_year(year);
...
double t_gtime::dmjd_vlbi(const bool &conv) const
{
    ...
    double tmjd = jd + int(30.6001 * (m + 1)) + b + 1720996.5 + day() + hour() / 24.0 + mins() / 1440.0 +
                  secs() / 86400.0 + dsec() / 86400.0;
    return tmjd;
}
...
{
    _mjd_conv =
        _mjd + floor((_sod + tai_tsys(_tsys) + _dsec + tai_tsys_dsec(_tsys)) / 86400.0); // 86400.0  have to be float!
