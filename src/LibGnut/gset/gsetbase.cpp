...
t_gsetbase::t_gsetbase() : _name(""), _pgm(""), _ver(""), _rev(""), _own(""), _dat(""), _tim("")
{
...
void t_gsetbase::arg(int argc, char *argv[], bool add, bool thin)
{
    ...
    for (int i = 1; i < argc; ++i)
    {
                ...
                (pos1 = s.find('=', pos2)) != string::npos))  // ORDER IMPORTANT
        {
            ...
            {
                if (isAttr)
                    _default_attr(NODE, s.substr(pos1 + 1, pos2 - pos1 - 1).c_str(), s.substr(pos2 + 1, string::npos),
                                  true);
                else if (isNode)
                    _default_node(NODE, s.substr(pos1 + 1, pos2 - pos1 - 1).c_str(),
                                  s.substr(pos2 + 1, string::npos).c_str(), true); // jdhuang
                else
                    _default_node(NODE, s.substr(pos1 + 1, pos2 - pos1 - 1).c_str(),
                                  s.substr(pos2 + 1, string::npos).c_str(), true);
...
void t_gsetbase::app(const string &pgm, const string &ver, const string &rev, const string &own, const string &dat,
                     const string &tim)
{
    _gmutex.lock();
