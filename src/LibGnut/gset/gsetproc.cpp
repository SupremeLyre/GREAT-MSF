...
t_gsetproc::t_gsetproc() : t_gsetbase()
{
...
OBSWEIGHT t_gsetproc::str2obsweight(const string &wg)
{
    ...
    {
        stringstream ostr;
        ostr << "Unsupported observation weighting model (" << wg << ")! Used default value ("
             << obsweight2str(_obs_weight) << ")";
        _add_log("gsetproc", ostr.str());
...
OBSCOMBIN t_gsetproc::str2obscombin(const string &oc)
{
    ...
    {
        stringstream ostr;
        ostr << "Unsupported observations combination (" << oc << ")! Used default value ("
             << obscombin2str(_obs_combin) << ")";
        _add_log("gsetproc", ostr.str());
...
t_gobscombtype::t_gobscombtype()
    : _obs_type(GOBSTYPE::TYPE), _obs_band_1(GOBSBAND::BAND), _obs_combine(OBSCOMBIN::DEF_OBSCOMBIN)
{
}

t_gobscombtype::t_gobscombtype(const t_gobscombtype &other)
    : _obs_type(other._obs_type), _obs_band_1(other._obs_band_1), _obs_band_2(other._obs_band_2),
      _obs_freq_1(other._obs_freq_1), _obs_freq_2(other._obs_freq_2), _obs_combine(other._obs_combine)
{
}
...
t_gobscombtype::t_gobscombtype(const t_gobs &obstype, OBSCOMBIN combtype)
    : _obs_type(obstype.type()), _obs_band_1(obstype.band()), _obs_combine(combtype)
{
}

t_gobscombtype::t_gobscombtype(const t_gobs &obstype, GOBSBAND b1, FREQ_SEQ freq_1, OBSCOMBIN combtype)
    : _obs_type(obstype.type()), _obs_band_1(b1), _obs_freq_1(freq_1), _obs_combine(combtype)
{
}

t_gobscombtype::t_gobscombtype(const t_gobs &obstype, GOBSBAND b1, GOBSBAND b2, FREQ_SEQ freq_1, FREQ_SEQ freq_2,
                               OBSCOMBIN combtype)
    : _obs_type(obstype.type()), _obs_band_1(b1), _obs_band_2(b2), _obs_freq_1(freq_1), _obs_freq_2(freq_2),
      _obs_combine(combtype)
{
}

t_gobscombtype::t_gobscombtype(GOBSTYPE t, GOBSBAND b, OBSCOMBIN obscomb)
    : _obs_type(t), _obs_band_1(b), _obs_combine(obscomb)
{
}
