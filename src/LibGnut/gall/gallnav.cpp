...
{
t_gallnav::t_gallnav()
    : t_gdata(), _com(false), _nepoch(t_gtime::GPS), _multimap(false), _overwrite(false), _chkHealth(true),
      _chkNavig(true), _chkTot(false)
{
...
t_gallnav::t_gallnav(t_spdlog spdlog)
    : t_gdata(spdlog), _com(false), _nepoch(t_gtime::GPS), _multimap(false), _overwrite(false), _chkHealth(true),
      _chkNavig(true), _chkTot(false)
{
    id_type(t_gdata::ALLNAV);
    id_group(t_gdata::GRP_EPHEM);
...
int t_gallnav::pos(const string &sat, const int &iod, const t_gtime &t, double xyz[3], double var[3], double vel[3],
                   const bool &chk_mask) // [m]
{
    _gmutex.lock();
