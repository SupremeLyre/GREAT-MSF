...
t_gallobs::t_gallobs() : t_gdata(), _set(0), _nepoch(0), _overwrite(false)
{
    id_type(t_gdata::ALLOBS);
    id_group(t_gdata::GRP_OBSERV);
}
t_gallobs::t_gallobs(t_spdlog spdlog) : t_gdata(spdlog), _set(0), _nepoch(0), _overwrite(false)
{
    id_type(t_gdata::ALLOBS);
    id_group(t_gdata::GRP_OBSERV);
}
t_gallobs::t_gallobs(t_spdlog spdlog, t_gsetbase *set) : t_gdata(spdlog), _set(set), _nepoch(0), _overwrite(false)
{
    if (nullptr == set)
    {
    ...
    if (fabs(outsync) < 0.014 && fabs(outsync) > 1e-6)
    {
        ...
        for (; itOBS != v_obs.end(); ++itOBS)
        {
            ...
            if (obstype == TYPE_P || obstype == TYPE_C)
                ...
                obs->resetobs(*itOBS,
                              obs->getobs(*itOBS) - CLIGHT * outsync / obs->wavelength(str2gobsband(gobs2str(*itOBS))));
