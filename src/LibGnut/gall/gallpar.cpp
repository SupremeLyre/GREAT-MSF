...
int t_gallpar::getParam(const string &mark, const par_type &type, const string &prn, const t_gtime &beg,
...
vector<int> t_gallpar::getPartialIndex(const string &site, const string &sat)
{
    ...
    vector<int> ans;
    pair<string, string> type_list[4] = {make_pair(site, sat), make_pair(site, ""), make_pair("", sat),
                                         make_pair("", "")};
...
vector<int> t_gallpar::delAmb()
{
    ...
    while (iter != _vParam.end())
    {
        if (iter->parType == par_type::AMB_IF || iter->parType == par_type::AMB_L1 ||
            ...
            iter->parType == par_type::AMB_L4 || iter->parType == par_type::AMB_L5 || iter->parType == par_type::AMB_WL)
        {
...
ostream &operator<<(ostream &os, t_gallpar &x)
{
    ...
    {
        if (x[i].parType == par_type::AMB_IF || x[i].parType == par_type::AMB_L1 || x[i].parType == par_type::AMB_L2 ||
            x[i].parType == par_type::AMB_L3 || x[i].parType == par_type::AMB_L4 || x[i].parType == par_type::AMB_L5 ||
