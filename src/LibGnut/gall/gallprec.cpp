...
t_gallprec::t_gallprec()
    : t_gallnav(), _degree_sp3(9), _sec(3600.0 * 6), _ref(t_gtime::GPS), _clkref(t_gtime::GPS), _clkrnx(true),
      _clksp3(false), _clknav(false), _posnav(false)
{
...
t_gallprec::t_gallprec(t_spdlog spdlog)
    : t_gallnav(spdlog), _degree_sp3(9), _sec(3600.0 * 6), _ref(t_gtime::GPS), _clkref(t_gtime::GPS), _clkrnx(true),
      _clksp3(false), _clknav(false), _posnav(false)
{
...
int t_gallprec::pos(const string &sat, const t_gtime &t, double xyz[3], double var[3], double vel[3],
                    const bool &chk_mask)
{
    _gmutex.lock();
...
int t_gallprec::addclk(const string &sat, const t_gtime &ep, double clk[3], double dxyz[3])
{
    ...
    if (_overwrite || _mapclk[sat].find(ep) == _mapclk[sat].end())
    {
        ...
        t_map_dat data = {
            {"C0", clk[0]}, {"C1", clk[1]}, {"C2", clk[2]}, {"SC0", dxyz[0]}, {"SC1", dxyz[1]}, {"SC2", dxyz[2]},
        };
        _mapclk[sat][ep] = data;
...
int t_gallprec::addclk_tri(const string &sat, const t_gtime &ep, double clk[3], double dxyz[3])
{
    ...
    if (_overwrite || _mapclk[sat].find(ep) == _mapclk[sat].end())
    {
        ...
        t_map_dat data = {
            {"C0", clk[0]},   {"C1", clk[1]},   {"C2", clk[2]},   {"IFCB_F3", clk[3]},
            {"SC0", dxyz[0]}, {"SC1", dxyz[1]}, {"SC2", dxyz[2]},
        };
        _mapclk[sat][ep] = data;
...
int t_gallprec::nav(const string &sat, const t_gtime &t, double xyz[3], double var[3], double vel[3],
                    const bool &chk_mask)
{
