...
class LibGnut_LIBRARY_EXPORT t_gsppflt : public virtual t_gspp
{
  ...
  protected:
    ...
    int _addObsP(t_gsatdata &satdata, unsigned int &iobs, t_gtriple &ell, Matrix &A, ColumnVector &l,
                 DiagonalMatrix &P);

    /** @brief add data to A, l, P - carrier phase. */
    virtual int _addObsL(t_gsatdata &satdata, unsigned int &iobs, t_gtriple &ell, Matrix &A, ColumnVector &l,
                         DiagonalMatrix &P);
