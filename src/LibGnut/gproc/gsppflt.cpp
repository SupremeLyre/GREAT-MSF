...
t_gsppflt::t_gsppflt(string mark, t_gsetbase *set)
    : t_gspp(mark, set), _minsat(static_cast<size_t>(SPP_MINSAT)), _filter(0), _smooth(false), _n_NPD_flt(0),
      _n_ALL_flt(0), _n_NPD_smt(0), _n_ALL_smt(0)
{
...
t_gsppflt::t_gsppflt(string mark, t_gsetbase *set, t_spdlog spdlog, string mode)
    : t_gspp(mark, set, spdlog, mode), _minsat(static_cast<size_t>(SPP_MINSAT)), _filter(0), _smooth(false),
      _n_NPD_flt(0), _n_ALL_flt(0), _n_NPD_smt(0), _n_ALL_smt(0)
{
    t_gspp::_get_settings();
    string fltModStr(dynamic_cast<t_gsetflt *>(_set)->method_flt());
    if (fltModStr.compare("kalman") == 0)
...
int t_gsppflt::_processEpoch(const t_gtime &runEpoch)
{
    ...
    {
        if (_spdlog)
            SPDLOG_LOGGER_DEBUG(_spdlog, _site + _epoch.str_ymdhms(" epoch ") + " skipped: " + int2str(_data.size()) +
                                             " < _minsat)");
        _restore(QsavBP, XsavBP);
        return -1;
    }
...
int t_gsppflt::_addObsP(t_gsatdata &satdata, unsigned int &iobs, t_gtriple &ell, Matrix &A, ColumnVector &l,
                        DiagonalMatrix &P)
{
...
int t_gsppflt::_addObsL(t_gsatdata &satdata, unsigned int &iobs, t_gtriple &ell, Matrix &A, ColumnVector &l,
                        DiagonalMatrix &P)
{
...
int t_gsppflt::_prepareData()
{
    ...
    {
        if (_spdlog)
            SPDLOG_LOGGER_DEBUG(_spdlog, _epoch.str_ymdhms(_site + " epoch ") + " skipped (Bancroft not calculated: " +
                                             int2str(_data.size()) + " < _minsat)");
        return -1;
    }
    ...
    if (BB.Nrows() < static_cast<int>(_minsat))
    {
        ...
        if (_spdlog)
            SPDLOG_LOGGER_DEBUG(_spdlog, _epoch.str_ymdhms(_site + " epoch ") +
    ...
    if (radius < B_WGS - 500)
    {
        string warning = "WARNING: Unexpected site (" + _site +
    ...
    while (iter != _data.end())
    {
        ...
        if (iter->ele_deg() < _minElev)
        {
            ...
            {
                if (_spdlog)
                    SPDLOG_LOGGER_DEBUG(_spdlog, _site + _epoch.str_ymdhms(" epoch ") + int2str(_data.size()) +
                                                     " skipped (data.size < _minsat)");
                return -1;
            }
            continue;
        }
