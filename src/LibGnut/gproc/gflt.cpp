...
void t_gflt::add_data(const t_gallpar &param, const ColumnVector &dx, const SymmetricMatrix &Qx, const double &sigma0,
...
void t_gflt::add_virtual_obs(const Matrix &A, const SymmetricMatrix &P, const ColumnVector &l)
{
    ...
    _P_virtual.SubMatrix(virtual_total_obs_num + 1, virtual_total_obs_num + virtual_obs_num, virtual_total_obs_num + 1,
...
bool t_gflt::outlierDetect(const Matrix &A, const SymmetricMatrix &Q, const ColumnVector &dx, const ColumnVector &l,
                           SymmetricMatrix &P)
{
    bool res = false;
    double k0 = 3, k1 = 7;
...
void t_kalman::update(const Matrix &A, const DiagonalMatrix &Pl, const ColumnVector &l, ColumnVector &dx,
                      SymmetricMatrix &Qx)
{
...
void t_kalman::update(const Matrix &A, const SymmetricMatrix &Pl, const ColumnVector &l, ColumnVector &dx,
                      SymmetricMatrix &Qx)
{
    Matrix K; // Kalman gain
    Matrix NN;
...
void t_SRF::update(const Matrix &A, const DiagonalMatrix &Pl, const ColumnVector &l, ColumnVector &dx,
                   SymmetricMatrix &Qx)
{
    int nObs = A.Nrows();
    int nPar = A.Ncols();
...
void t_SRF::update(const Matrix &A, const SymmetricMatrix &Pl, const ColumnVector &l, ColumnVector &dx,
                   SymmetricMatrix &Qx)
{
...
void t_SRIF::update(const Matrix &A, const DiagonalMatrix &Pl, const ColumnVector &l, ColumnVector &dx,
                    SymmetricMatrix &Qx)
{
    int nObs = A.Nrows();
    int nPar = A.Ncols();
