...
namespace gnut
{
t_gpreproc::t_gpreproc(t_gallobs *obs, t_gsetbase *settings) : _spdlog(nullptr)
{
    if (nullptr == settings)
    {
...
int t_gpreproc::ProcessBatch(string site, const t_gtime &beg_r, const t_gtime &end_r, double sampl, bool sync,
                             bool save)
{
    ...
    if (bands_t1.size() <= 1)
    {
        if (_spdlog)
            SPDLOG_LOGGER_DEBUG(_spdlog,
                                gobs1->epoch().str_ymdhms("Not enough bands available: ") + " " + gobs1->sat());
        return -1;
    }
...
t_gobs_pair::t_gobs_pair(t_gobs &gobs1, t_gobs &gobs2) : obs1(gobs1), obs2(gobs2)
{
}
