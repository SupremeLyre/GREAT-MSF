...
t_gppp::t_gppp(string mark, t_gsetbase *set) : t_gspp(mark, set), _gotl(0), _running(false)
{
...
t_gppp::t_gppp(string mark, t_gsetbase *set, t_spdlog spdlog) : t_gspp(mark, set, spdlog), _gotl(0), _running(false)
{
    _tides = new t_gtide2010(_spdlog);
    _gModel = new t_gpppmodel(_site, _spdlog, _set);
    ...
    for (it = rslt.v_rslt.begin(); it != rslt.v_rslt.end(); ++it)
    {
        ...
        if (it->type == "TRP" && xyz[0] != 0.0)
        {
            ...
            os << fixed << setprecision(3) << tt.str_ymdhms() << " " << setw(13) << xyz[0] << " " << setw(13) << xyz[1]
               << " " << setw(13) << xyz[2] << " " << setw(7) << ztd << " " << setw(13) << neu[0] << " " << setw(13)
               << neu[1] << " " << setw(13) << neu[2] << fixed << setprecision(5) << " " << setw(9) << rms[0] << " "
               << setw(9) << rms[1] << " " << setw(9) << rms[2] << " " << setw(7) << ztdrms << " " << setw(2)
               << 0 // DATA SIZE NOT KNOWN HERE !!!!!
               << endl;
