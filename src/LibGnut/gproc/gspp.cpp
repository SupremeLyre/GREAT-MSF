...
t_gspp::t_gspp(string mark, t_gsetbase *set)
    : _grec(nullobj), _gallobj(0), _gallbias(0), _weight(OBSWEIGHT::SINEL), _observ(OBSCOMBIN::IONO_FREE),
      _valid_crd_xml(false), _valid_ztd_xml(false), _site(mark), _set(set), _res(0), _gobs(0), _gnav(0), _gmet(0),
      _gion(0), _allprod(0), _phase(false), _doppler(false), _gnss(GNS), _initialized(false), _use_ecl(false)
{
    if (nullptr == set)
    {
