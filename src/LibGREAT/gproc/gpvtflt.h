...
class LibGREAT_LIBRARY_EXPORT t_gpvtflt : public t_gpppflt
{
  public:
     ...
     */
    virtual int ProcessOneEpoch(const t_gtime &now, vector<t_gsatdata> *data_rover = NULL,
                                vector<t_gsatdata> *data_base = NULL);
     ...
     * @return void
     */
    void Add_rho_azel(/*const t_gtime &runEpoch, */ const string &site_name, t_gtriple &xyz_s, const t_gtriple &xyz_r,
                      t_gsatdata &obs_sat);
  ...
  protected:
     ...
     */
    bool _valid_residual(bool phase_process, string sat_name, enum FREQ_SEQ &f,
     ...
     * @return retval > 0,success; retval < 0,fail
     */
    int _addObsD(t_gsatdata &satdata, unsigned int &iobs, t_gallpar &param, t_gtriple &XYZ, Matrix &A, ColumnVector &l,
                 DiagonalMatrix &P);
     ...
     */
    virtual void _prtOut(t_gtime &epo, t_gallpar &X, const SymmetricMatrix &Q, vector<t_gsatdata> &data,
                         ostringstream &os, xml_node &node, bool saveProd = true);
    ...
    void _posterioriTest(const Matrix &A, const SymmetricMatrix &P, const ColumnVector &l, const ColumnVector &dx,
                         const SymmetricMatrix &Q, ColumnVector &v_norm, double &vtpv);
