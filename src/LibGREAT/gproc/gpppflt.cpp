...
t_gpppflt::t_gpppflt(string mark, t_gsetbase *gset)
    : t_gspp(mark, gset), t_gppp(mark, gset), t_gsppflt(mark, gset), t_gxml("kml"), _read(false), _flt(0), _kml(false),
      _beg_end(true)
{
...
t_gpppflt::t_gpppflt(string mark, t_gsetbase *gset, t_spdlog spdlog)
    : t_gspp(mark, gset, spdlog), t_gppp(mark, gset, spdlog), t_gsppflt(mark, gset, spdlog), t_gxml("kml"),
      _read(false), _flt(0), _kml(false), _beg_end(true)
{
    this->_setOut();
    _epoch.tsys(t_gtime::GPS);
    _grdStoModel = new t_randomwalk();
...
int t_gpppflt::_apply_tides(t_gtime &_epoch, t_gtriple &xRec)
{
    if (_tides)
    {
        xRec = xRec + _tides->tide_searth(_epoch, xRec) + _tides->load_ocean(_epoch, _site, xRec) +
               _tides->tide_pole() + _tides->load_atmosph();
    }
    else
        return -1;
    return 1;
