...
void t_gfltEquationMatrix::add_equ(const vector<pair<int, double>> &B_value, const double &P_value,
                                   const double &l_value, const string &stie_name, const string &sat_name,
                                   const t_gobscombtype &obscombtype, const bool &is_newamb)
{
    B.push_back(B_value);
    P.push_back(P_value);
    l.push_back(l_value);
...
void t_gfltEquationMatrix::add_equ(const t_gfltEquationMatrix &Other)
{
    ...
    {
        this->add_equ(Other.B[i], Other.P[i], Other.l[i], Other._site_sat_pairlist[i].first,
                      Other._site_sat_pairlist[i].second, Other._obstypelist[i], false);
    }
}

void t_gfltEquationMatrix::chageNewMat(Matrix &B_value, SymmetricMatrix &P_value, ColumnVector &l_value,
                                       const int &par_num)
{
