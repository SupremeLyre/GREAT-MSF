...
t_gbds_codebias_cor::t_gbds_codebias_cor(t_gsetbase *settings)
{
    ...
    // set Wanninger & Beer
    _IGSO_MEO_Corr[BAND_2]["IGSO"] = {{0, -0.55}, {1, -0.40}, {2, -0.34}, {3, -0.23}, {4, -0.15},
                                      {5, -0.04}, {6, 0.09},  {7, 0.19},  {8, 0.27},  {9, 0.35}}; // B1
    _IGSO_MEO_Corr[BAND_7]["IGSO"] = {{0, -0.71}, {1, -0.36}, {2, -0.33}, {3, -0.19}, {4, -0.14},
                                      {5, -0.03}, {6, 0.08},  {7, 0.17},  {8, 0.24},  {9, 0.33}}; // B2
    _IGSO_MEO_Corr[BAND_6]["IGSO"] = {{0, -0.27}, {1, -0.23}, {2, -0.21}, {3, -0.15}, {4, -0.11},
                                      {5, -0.04}, {6, 0.05},  {7, 0.14},  {8, 0.19},  {9, 0.32}}; // B3

    _IGSO_MEO_Corr[BAND_2]["MEO"] = {{0, -0.47}, {1, -0.38}, {2, -0.32}, {3, -0.23}, {4, -0.11},
                                     {5, 0.06},  {6, 0.34},  {7, 0.69},  {8, 0.97},  {9, 1.05}}; // B1
    _IGSO_MEO_Corr[BAND_7]["MEO"] = {{0, -0.40}, {1, -0.31}, {2, -0.26}, {3, -0.18}, {4, -0.06},
                                     {5, 0.09},  {6, 0.28},  {7, 0.48},  {8, 0.64},  {9, 0.69}}; // B2
    _IGSO_MEO_Corr[BAND_6]["MEO"] = {{0, -0.22}, {1, -0.15}, {2, -0.13}, {3, -0.10}, {4, -0.04},
                                     {5, 0.05},  {6, 0.14},  {7, 0.27},  {8, 0.36},  {9, 0.47}}; // B3
}

void t_gbds_codebias_cor::apply_IGSO_MEO(const string &rec, t_gtriple &rec_crd, t_gallnav *gnav,
                                         vector<t_gsatdata> &obsdata)
{
    ...
    for (data = obsdata.begin(); data != obsdata.end();)
    {
        ...
        double elev =
            (rec_crd[0] * rec_sat_vector[0] + rec_crd[1] * rec_sat_vector[1] + rec_crd[2] * rec_sat_vector[2]) /
...
bool t_gbds_codebias_cor::_recAprCoordinate(const string &rec, t_gtriple &rec_crd, t_gallnav *gnav,
                                            vector<t_gsatdata> &obsdata)
{
    Matrix BB;
...
t_gqualitycontrol::t_gqualitycontrol(t_spdlog spdlog, t_gsetbase *settings, t_gallnav *gnav)
    : _gnav(gnav), _spdlog(spdlog), _bds_codebias_cor(settings), _smooth_range(settings),
...
t_gqualitycontrol::t_gqualitycontrol(t_gsetbase *settings, t_gallnav *gnav)
    : _gnav(gnav), _bds_codebias_cor(settings), _smooth_range(settings), _outliers_proc(settings, nullptr)
{
}
...
int t_gqualitycontrol::processOneEpoch(const t_gtime &now, const string &rec, t_gtriple &rec_crd,
                                       vector<t_gsatdata> &obsdata)
{
