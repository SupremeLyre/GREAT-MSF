...
class LibGREAT_LIBRARY_EXPORT t_gtrs2crs
{
  ...
  protected:
    ...
    /** @brief calculate the diurnal and semi-diurnal variations in Earth Orientation Parameters (x,y, UT1, LOD) from
     * ocean tides (fortran: interp.f). */
    /** agree with _ORTHO_EOP at the level of a few microarcseconds in polar motion and a few tenths of a microsecond in
     * UT1*/
    ...
    void _process2000(const double &dRmjd, const double &dpsi, const double &deps, Matrix &qmat, const bool &ldpsi,
                      const bool &ldeps);

    /** @brief calculate rotation matrix of IAU2006/2000A Precession and nutation. */
    void _process2006(const double &dRmjd, const double &dpsi, const double &deps, Matrix &qmat, const bool &ldpsi,
                      const bool &ldeps);
    ...
    const double dPrecor =
        -0.29965 / (RAD2SEC); ///< The precession and obliquity corrections (radians per century) (page 43, equ(27))
    const double dOblcor = -0.02524 / (RAD2SEC); ///< The obliquity corrections (radians per century) (page 43, equ(27))
    const double dPsibi =
        -0.041775 / (RAD2SEC); ///< The frame bias corrections in longitude and obliquity (page 43, equ(28) ?)
    const double dEpsbi =
        -0.0068192 / (RAD2SEC); ///< The eps bias corrections in longitude and obliquity (page 43, equ(28) ?)
