...
class LibGREAT_LIBRARY_EXPORT t_gambiguity
{
  public:
     ...
     */
    double lambdaSolve(double &ratio, const Matrix &anor, const ColumnVector &fltpar, ColumnVector &ibias,
                       bool parlamb = false);
  ...
  protected:
    ...
    /**
     * @brief check if a new ambiguity is dependant of the already selected set expressed by a set of orth. unit vector
     * Ei.
     ...
     * @return     true - compute successfully , false - failure
     */
    bool _checkAmbDepend(bool isFirst, int iNamb, int *iNdef, int iN_oneway, int *arriIpt2ow, int iMaxamb_ow,
