...
namespace great
{
int t_gattitude_model::attitude_old(t_gsatdata &satdata, string antype, Eigen::Vector3d &i, Eigen::Vector3d &j,
                                    Eigen::Vector3d &k)
{
    if (satdata.gsys() == GAL)
    {
...
int t_gattitude_model::attitude(t_gsatdata &satdata, double yaw, Eigen::Vector3d &i, Eigen::Vector3d &j,
                                Eigen::Vector3d &k)
{
...
int t_gattitude_model::attitude(t_gsatdata &satdata, string antype, Eigen::Vector3d &i, Eigen::Vector3d &j,
                                Eigen::Vector3d &k)
{
...
int t_gattitude_model::attitude(string antype, string prn, ColumnVector &xsat, ColumnVector &vsat, ColumnVector &xsun,
                                ColumnVector &i, ColumnVector &j, ColumnVector &k)
{
...
void t_gattitude_model::_ysm(string prn, double bata, double mi, ColumnVector &xsat, ColumnVector &vsat,
                             ColumnVector &xsun, ColumnVector &i, ColumnVector &j, ColumnVector &k)
{
    ColumnVector satcrd = xsat;
...
void t_gattitude_model::_onm(ColumnVector &xsat, ColumnVector &vsat, ColumnVector &xsun, ColumnVector &i,
                             ColumnVector &j, ColumnVector &k)
{
    double yaw = 0;
    _yaw2ijk(xsat, vsat, xsun, yaw, i, j, k);
}

int t_gattitude_model::_yaw(t_gsatdata &satdata, string antype, Eigen::Vector3d &xs, Eigen::Vector3d &ys,
                            Eigen::Vector3d &zs)
{
#if 0
...
void t_gattitude_model::_attitude_GPSIIA(t_gsatdata &satdata, string antype, Eigen::Vector3d &i, Eigen::Vector3d &j,
                                         Eigen::Vector3d &k)
{
...
void t_gattitude_model::_attitude_GPSIIA(string antype, string prn, ColumnVector &xsat, ColumnVector &vsat,
                                         ColumnVector &xsun, ColumnVector &i, ColumnVector &j, ColumnVector &k)
{
    const double R_GPSIIA[] = {0.1046, 0.1230, 0.1255, 0.1249, 0.1003, 0.1230, 0.1136, 0.1169, 0.1253, 0.0999, 0.1230,
                               0.1230, 0.1230, 0.1230, 0.1092, 0.1230, 0.1230, 0.1230, 0.1230, 0.1230, 0.1230, 0.1230,
...
void t_gattitude_model::_attitude_GPSIIR(t_gsatdata &satdata, string antype, Eigen::Vector3d &i, Eigen::Vector3d &j,
                                         Eigen::Vector3d &k)
{
...
void t_gattitude_model::_attitude_GPSIIR(string antype, string prn, ColumnVector &xsat, ColumnVector &vsat,
                                         ColumnVector &xsun, ColumnVector &i, ColumnVector &j, ColumnVector &k)
{
    const double R = 0.2 * D2R; // maximal yaw hardware rate
    double beta0 = atan2(MUDOT_GPS, R);
...
void t_gattitude_model::_noon_turn(t_gsatdata &satdata, double R, Eigen::Vector3d &i, Eigen::Vector3d &j,
                                   Eigen::Vector3d &k)
{
    double beta0 = atan2(MUDOT_GPS, R);
...
void t_gattitude_model::_midnight_turn(t_gsatdata &satdata, double R, Eigen::Vector3d &i, Eigen::Vector3d &j,
                                       Eigen::Vector3d &k)
{
    double beta0 = atan2(MUDOT_GPS, R);
    double beta = satdata.beta();
...
void t_gattitude_model::_noon_turn(string _prn, double _beta, double _mi, ColumnVector &xsat, ColumnVector &vsat,
                                   ColumnVector &xsun, double R, ColumnVector &i, ColumnVector &j, ColumnVector &k)
{
    double beta0 = atan2(MUDOT_GPS, R);
    double beta = _beta;
    double mi = _mi;
...
void t_gattitude_model::_midnight_turn_GPSIIA(t_gsatdata &satdata, double R, Eigen::Vector3d &i, Eigen::Vector3d &j,
                                              Eigen::Vector3d &k)
{
    double beta = satdata.beta();
    double mi = satdata.orb_angle();
...
void t_gattitude_model::_midnight_turn_GPSIIA(string prn, double _beta, double _mi, ColumnVector &xsat,
                                              ColumnVector &vsat, ColumnVector &xsun, double R, ColumnVector &i,
                                              ColumnVector &j, ColumnVector &k)
{
    double beta = _beta;
    double mi = _mi;
...
void t_gattitude_model::_midnight_turn_GPSIIF(t_gsatdata &satdata, double R, Eigen::Vector3d &i, Eigen::Vector3d &j,
                                              Eigen::Vector3d &k)
{
    double beta = satdata.beta();
...
void t_gattitude_model::_midnight_turn_GPSIIF(string prn, double _beta, double _mi, ColumnVector &xsat,
                                              ColumnVector &vsat, ColumnVector &xsun, double R, ColumnVector &i,
                                              ColumnVector &j, ColumnVector &k)
{
    double beta = _beta;
...
void t_gattitude_model::_midnight_turn_GLOM(t_gsatdata &satdata, double R, Eigen::Vector3d &i, Eigen::Vector3d &j,
                                            Eigen::Vector3d &k)
{
    double beta = satdata.beta();
    double tan_beta = tan(beta);
...
void t_gattitude_model::_midnight_turn_GLOM(string prn, double _beta, double _mi, ColumnVector &xsat,
                                            ColumnVector &vsat, ColumnVector &xsun, double R, ColumnVector &i,
                                            ColumnVector &j, ColumnVector &k)
{
    double beta = _beta;
...
void t_gattitude_model::_noon_turn_GLOM(t_gsatdata &satdata, double R, Eigen::Vector3d &i, Eigen::Vector3d &j,
                                        Eigen::Vector3d &k)
{
    double beta = satdata.beta();
    double mi = satdata.orb_angle();
...
void t_gattitude_model::_noon_turn_GLOM(string prn, double _beta, double _mi, ColumnVector &xsat, ColumnVector &vsat,
                                        ColumnVector &xsun, double R, ColumnVector &i, ColumnVector &j, ColumnVector &k)
{
    double beta = _beta;
    double mi = _mi;
...
void t_gattitude_model::_attitude_GPSIIRM(t_gsatdata &satdata, string antype, Eigen::Vector3d &i, Eigen::Vector3d &j,
                                          Eigen::Vector3d &k)
{
    _attitude_GPSIIR(satdata, antype, i, j, k);
}

void t_gattitude_model::_attitude_GPSIIF(t_gsatdata &satdata, string antype, Eigen::Vector3d &i, Eigen::Vector3d &j,
                                         Eigen::Vector3d &k)
{
...
void t_gattitude_model::_attitude_GPSIIF(string antype, string prn, ColumnVector &xsat, ColumnVector &vsat,
                                         ColumnVector &xsun, ColumnVector &i, ColumnVector &j, ColumnVector &k)
{
    const double R_noon = 0.11 * D2R; // maximal yaw hardware rate during noon turn
    const double R_midn = 0.06 * D2R; // maximal yaw hardware rate during midnight turn
...
void t_gattitude_model::_attitude_GPSIII(t_gsatdata &satdata, string antype, Eigen::Vector3d &i, Eigen::Vector3d &j,
                                         Eigen::Vector3d &k)
{
    // to be added;
    _ysm(satdata, i, j, k);
...
void t_gattitude_model::_attitude_GPSIII(string antype, string prn, ColumnVector &xsat, ColumnVector &vsat,
                                         ColumnVector &xsun, ColumnVector &i, ColumnVector &j, ColumnVector &k)
{
}

void t_gattitude_model::_attitude_GLO(t_gsatdata &satdata, string antype, Eigen::Vector3d &i, Eigen::Vector3d &j,
                                      Eigen::Vector3d &k)
{
...
void t_gattitude_model::_attitude_GLO(string antype, string prn, ColumnVector &xsat, ColumnVector &vsat,
                                      ColumnVector &xsun, ColumnVector &i, ColumnVector &j, ColumnVector &k)
{
    const double R = 0.25 * D2R; // maximal yaw hardware rate
    double beta0 = 2.0 * D2R;
...
void t_gattitude_model::_noon_turn_GAL1(string prn, double _beta, double _mi, ColumnVector &xsat, ColumnVector &vsat,
                                        ColumnVector &xsun, ColumnVector &i, ColumnVector &j, ColumnVector &k)
{
    double beta = _beta;
    double mi = _mi;
...
void t_gattitude_model::_attitude_GAL1(t_gsatdata &satdata, string antype, Eigen::Vector3d &i, Eigen::Vector3d &j,
                                       Eigen::Vector3d &k)
{
    double beta0 = 2.0 * D2R;
...
void t_gattitude_model::_attitude_GAL1(string antype, string prn, ColumnVector &xsat, ColumnVector &vsat,
                                       ColumnVector &xsun, ColumnVector &i, ColumnVector &j, ColumnVector &k)
{
    double beta0 = 2.0 * D2R;
...
void t_gattitude_model::_noon_turn_GAL2(string prn, double _beta, double _mi, ColumnVector &xsat, ColumnVector &vsat,
                                        ColumnVector &xsun, ColumnVector &i, ColumnVector &j, ColumnVector &k)
{
    ...
    if (fabs(beta) < 4.1 * D2R && fabs(du) < 18.2 * D2R)
    {
        ...
        yaw = 0.5 * G_PI * sign(1, yangle0) +
              (yangle0 - 0.5 * G_PI * sign(1, yangle0)) * cos(fabs(2 * G_PI / 5656 * tmod));
    }
    else
    {
        yaw = yawangle;
...
void t_gattitude_model::_attitude_GAL2(t_gsatdata &satdata, string antype, Eigen::Vector3d &i, Eigen::Vector3d &j,
                                       Eigen::Vector3d &k)
{
...
void t_gattitude_model::_attitude_GAL2(string antype, string prn, ColumnVector &xsat, ColumnVector &vsat,
                                       ColumnVector &xsun, ColumnVector &i, ColumnVector &j, ColumnVector &k)
{
    double beta0 = 4.1 * D2R;
    double mi_n0 = (180 - 10) * D2R;
...
void t_gattitude_model::_cys_cast(t_gsatdata &satdata, Eigen::Vector3d &i, Eigen::Vector3d &j, Eigen::Vector3d &k)
{
    ...
    if (fabs(satdata.beta()) <= beta_threshold_cast * D2R)
    {
        ...
        double beta_modified =
            satdata.beta() + f_function * (sign(beta_threshold_cast * D2R, satdata.beta()) - satdata.beta());
...
void t_gattitude_model::_cys_cast(string prn, double _beta, double _mi, ColumnVector &xsat, ColumnVector &vsat,
                                  ColumnVector &xsun, ColumnVector &i, ColumnVector &j, ColumnVector &k)
{
    double beta_threshold_cast = 2.8;
    double d_constant = 80000.0;
...
void t_gattitude_model::_cys_secm(string prn, double _beta, double _mi, ColumnVector &xsat, ColumnVector &vsat,
                                  ColumnVector &xsun, ColumnVector &i, ColumnVector &j, ColumnVector &k)
{
    double beta_threshold_secm = 3.0;
...
void t_gattitude_model::_attitude_BDS(t_gsatdata &satdata, string antype, Eigen::Vector3d &i, Eigen::Vector3d &j,
                                      Eigen::Vector3d &k)
{
...
void t_gattitude_model::_attitude_BDS(string antype, string prn, ColumnVector &xsat, ColumnVector &vsat,
                                      ColumnVector &xsun, ColumnVector &i, ColumnVector &j, ColumnVector &k)
{
    double orb_angle = _orb_angle(xsat, vsat, xsun);
    double beta = _beta(xsat, vsat, xsun);
...
void t_gattitude_model::_cys_qzs(string prn, double _beta, double _mi, ColumnVector &xsat, ColumnVector &vsat,
                                 ColumnVector &xsun, ColumnVector &i, ColumnVector &j, ColumnVector &k)
{
    // genernal settings and thresholds
    double beta_threshold_qzs = 5.0 * D2R; // radius
...
void t_gattitude_model::_attitude_QZS(t_gsatdata &satdata, string antype, Eigen::Vector3d &i, Eigen::Vector3d &j,
                                      Eigen::Vector3d &k)
{
...
void t_gattitude_model::_attitude_QZS(string antype, string prn, ColumnVector &xsat, ColumnVector &vsat,
                                      ColumnVector &xsun, ColumnVector &i, ColumnVector &j, ColumnVector &k)
{
    double orb_angle = _orb_angle(xsat, vsat, xsun);
    double beta = _beta(xsat, vsat, xsun);
...
void t_gattitude_model::_yaw2ijk(t_gsatdata &satdata, double &yaw, Eigen::Vector3d &i, Eigen::Vector3d &j,
                                 Eigen::Vector3d &k)
{
...
void t_gattitude_model::_yaw2ijk(ColumnVector &xsat, ColumnVector &vsat, ColumnVector &xsun, double &yaw,
                                 ColumnVector &i, ColumnVector &j, ColumnVector &k)
{
    // satdata.yaw(yaw);   // store yaw angle
    ColumnVector satcrd = xsat;
