...
t_gcombmodel::t_gcombmodel(t_gsetbase *setting, shared_ptr<t_gbiasmodel> bias_model, t_gallproc *data)
    : _bias_model(std::move(bias_model)), _spdlog(nullptr)
{
...
t_gcombmodel::t_gcombmodel(t_gsetbase *setting, t_spdlog spdlog, shared_ptr<t_gbiasmodel> bias_model, t_gallproc *data)
    : _bias_model(std::move(bias_model)), _spdlog(spdlog)
{
    _band_index[gnut::GPS] = dynamic_cast<t_gsetgnss *>(setting)->band_index(gnut::GPS);
    _band_index[gnut::GAL] = dynamic_cast<t_gsetgnss *>(setting)->band_index(gnut::GAL);
    _band_index[gnut::GLO] = dynamic_cast<t_gsetgnss *>(setting)->band_index(gnut::GLO);
...
t_gcombIF::t_gcombIF(t_gsetbase *setting, t_spdlog spdlog, shared_ptr<t_gbiasmodel> bias_model, t_gallproc *data)
    : t_gcombmodel(setting, spdlog, std::move(bias_model), data)
{
    _clk_type_index[make_pair(FREQ_3, GPS)] = par_type::CLK13_G;
    _clk_type_index[make_pair(FREQ_4, GPS)] = par_type::CLK14_G;
    _clk_type_index[make_pair(FREQ_5, GPS)] = par_type::CLK15_G;
...
bool t_gcombIF::cmb_equ_IF(t_gtime &epoch, t_gallpar &params, t_gsatdata &obsdata, GOBSBAND b1, GOBSBAND b2,
                           t_gbaseEquation &result)
{
    ...
    for (const auto &item : type_list)
    {
        ...
        for (int i = 0; i < temp_equ.B[0].size(); i++)
        {
            ...
            // combine coeff
            coef_IF.emplace_back(temp_equ.B[0][i].first,
                                 coef1 * temp_equ.B[0][i].second + coef2 * temp_equ.B[1][i].second);
        }
        // combine P and l
        ...
        {
            l_IF +=
                dynamic_cast<t_gprecisebias *>(_bias_model.get())->ifcbDelay(obsdata, nullptr, OBSCOMBIN::IONO_FREE);
        }
...
bool t_gcombIF::_add_IF_multi_rec_clk(const FREQ_SEQ &freq, t_gsatdata &obsdata, t_gallpar &params,
                                      vector<pair<int, double>> &coef_IF)
{
    auto gsys = obsdata.gsys();
    ...
    for (auto iter = coef_IF.begin(); iter != coef_IF.end();)
    {
        ...
        if (type == par_type::CLK || type == par_type::GAL_ISB || type == par_type::BDS_ISB ||
...
t_gcombALL::t_gcombALL(t_gsetbase *setting, t_spdlog spdlog, shared_ptr<t_gbiasmodel> bias_model, t_gallproc *data)
    : t_gcombmodel(setting, spdlog, std::move(bias_model), data)
{
    ambtype_list[FREQ_1] = par_type::AMB_L1;
    ambtype_list[FREQ_2] = par_type::AMB_L2;
...
bool t_gcombALL::cmb_equ(t_gtime &epoch, t_gallpar &params, t_gsatdata &obsdata, t_gbaseEquation &result)
{
    ...
    for (const auto &iter : crt_bands)
    {
        ...
        if (idx >= 0)
        {
            ...
            {
                tempL.l[0] +=
                    dynamic_cast<t_gprecisebias *>(_bias_model.get())->ifcbDelay(obsdata, nullptr, OBSCOMBIN::RAW_ALL);
            }
        }
        else
        {
...
t_gcombDD::t_gcombDD(t_gsetbase *setting, const shared_ptr<t_gbiasmodel> &bias_model, t_gallproc *data)
    : t_gcombmodel(setting, bias_model, data), t_gcombALL(setting, bias_model, data),
...
t_gcombDD::t_gcombDD(t_gsetbase *setting, t_spdlog spdlog, const shared_ptr<t_gbiasmodel> &bias_model, t_gallproc *data)
    : t_gcombmodel(setting, spdlog, bias_model, data), t_gcombALL(setting, spdlog, bias_model, data),
        ...
        for (int i = 0; i < type_list.size(); i++)
        {
            ...
            for (int j = 0; j < temp_equ.B[0].size(); j++)
            {
                ...
                if (temp_equ.B[0][j].first > npar_orig)
                    continue;
                coef_IF.emplace_back(temp_equ.B[0][j].first,
                                     coef1 * temp_equ.B[0][j].second + coef2 * temp_equ.B[1][j].second);
            }
            ...
            // combine l
            l_IF = coef1 * temp_equ.l[0] + coef2 * temp_equ.l[1] -
                   (coef1 * temp_equ_other.l[0] + coef2 * temp_equ_other.l[1]);
        ...
        for (int isite = 0; isite < 2; isite++)
        {
            ...
            for (const auto &iter : crt_bands)
            {
                ...
                if (obsP.type() != TYPE_C && obsP.type() != TYPE_P)
                {
                    if (_spdlog)
                        SPDLOG_LOGGER_INFO(_spdlog,
                                           "check your obs file, we have no range obs for " + gobsband2str(band));
                    continue;
                }
                ...
                if (obsL.type() != TYPE_L)
                {
                    if (_spdlog)
                        SPDLOG_LOGGER_INFO(_spdlog,
                                           "check your obs file, we have no phase obs for " + gobsband2str(band));
                    continue;
                }
                ...
                {
                    int idx = params_temp.getParam(
                        satdata_ptr->site(), ambtype_list[_freq_index[satdata_ptr->gsys()][band]], satdata_ptr->sat());
