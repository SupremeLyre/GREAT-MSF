...
t_gprecisebiasGPP::t_gprecisebiasGPP(t_gallproc *data, t_spdlog spdlog, t_gsetbase *setting)
    : t_gprecisebias(data, spdlog, setting)
{
    int base_size = dynamic_cast<t_gsetgen *>(setting)->list_base().size();
    _ddmode = (base_size ? true : false);
}
...
bool t_gprecisebiasGPP::cmb_equ(t_gtime &epoch, t_gallpar &params, t_gsatdata &obsdata, t_gobs &gobs,
...
double t_gprecisebiasGPP::cmpObs(t_gtime &epoch, string &sat, string &rec, t_gallpar &param, t_gsatdata &gsatdata,
                                 t_gobs &gobs)
{
    if (_gallobj == nullptr)
    {
...
bool t_gprecisebiasGPP::_prepare_obs_GPP(const t_gtime &crt_epo, t_gallnav *gallnav, t_gallobj *gallobj,
                                         t_gallpar &pars)
{
    if (!gallnav || !gallobj)
    {
    ...
    // add drate
    _crt_obs.adddrate(
        (DotProduct((_crs_sat_vel - _crs_rec_vel).crd_cvect(), (_crs_sat_pco - _crs_rec_pco).crd_cvect())) /
        (CLIGHT * tmp));
    ...
    t_gtriple xyz_s2r =
        t_gtriple(_rot_matrix_scf2crs.t() * ((-1) * xyz_rho.crd_cvect())); // from sat. to rec. in SCF XYZ
    ...
    double rho0 = sqrt(pow(_trs_rec_crd[0] - xyz_s[0], 2) + pow(_trs_rec_crd[1] - xyz_s[1], 2) +
                       pow(_trs_rec_crd[2] - xyz_s[2], 2));
...
bool t_gprecisebiasGPP::_update_obs_info_GPP(const t_gtime &epoch, t_gallnav *nav, t_gallobj *gallobj,
                                             t_gsatdata &obsdata, t_gallpar &pars)
{
    if (!nav || !gallobj)
    {
        if (_spdlog)
...
bool t_gprecisebiasGPP::_update_obj_clk_GPP(const string &obj, const t_gtime &crt_epoch, t_gallnav *nav,
...
bool t_gprecisebiasGPP::_prt_obs_ALL(const t_gtime &crt_epo, t_gsatdata &obsdata, t_gallpar &pars, t_gobs &gobs,
