...
void t_gnavde::_interp(const vector<double> &coeff, const int &ipt, const int &ncf, const int &ncm, const int &na,
                       const int &ifl, double dt[], double *pv)
{
    int np = 2;
    int nv = 3;
    double twot = 0.0;
    ...
    for (int i = 1; i <= ncm; i++)
    {
        ...
        {
            pv[(i - 1) * 2] =
                pv[(i - 1) * 2] + pc[j - 1] * coeff[ipt - 1 + (l - 1) * ncf * ncm + (i - 1) * ncf + j - 1];
        }
    }
    ...
    for (int i = 1; i <= ncm; i++)
    {
        ...
        {
            pv[(i - 1) * 2 + 6] =
                pv[(i - 1) * 2 + 6] + vc[j - 1] * coeff[ipt - 1 + (l - 1) * ncm * ncf + (i - 1) * ncf + j - 1];
        }
        pv[(i - 1) * 2 + 6] = pv[(i - 1) * 2 + 6] * vfac;
    }
}

void t_gnavde::add_head(double SS[3], const double &au, const double &emrat, int ipt[3][13],
                        const vector<string> &constname, const vector<double> &constval)
{
