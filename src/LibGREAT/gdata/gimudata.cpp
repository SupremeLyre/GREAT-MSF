...
bool great::t_gimudata::load(vector<Eigen::Vector3d> &wm, vector<Eigen::Vector3d> &vm, double &t, double &ts,
                             int nSamples, bool _beg_end)
{
...
bool great::t_gimudata::available(const t_gtime &now, bool _beg_end)
{
    try
    {
        ...
        while (_imu_forward.size() < 5)
            t_gtime::gmsleep(500);
        if ((_imu_forward.size() > 0 && _beg_end && t < _imu_forward.back().t) ||
            (_imu_back.size() > 0 && !_beg_end && t > _imu_back.front().t))
            return true;
        else
            return false;
    }
        ...
        while (target_t <= next_orig_t)
        {
            ...
            for (auto j = 0; j < 3; j++)
            {
                ...
                if (interp.linear(orig, target_t, imu01.wm[j]) < 0)
                {
                    ...
                    if (_spdlog)
                        SPDLOG_LOGGER_ERROR(_spdlog, "imu interpolation failed between " +
                                                         to_string(_imu_forward[i].t) + " and " +
                                                         to_string(_imu_forward[i + 1].t));
                }
                orig.clear();
